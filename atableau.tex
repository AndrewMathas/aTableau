...
% Modified: 10:45 Monday, 06 October 2025
% ----------------------------------------------------------------------
...
\begin{document}
  ...
  default options for the document can also be set via
  ...
  \begin{options}
    ...
    A shorthand for defining (single-use) \TikZ-styles for the current
    picture.
  ...
  \textcolor{orange}{false}, the key value can typically be omitted, and
  ...
  coordinates to position it inside a \keyword{tikzpicture} environment.
  Otherwise, these two \keyword{\Diagram} commands are identical, except
  that the second picture is embellished with labels that are drawn by
  ...
  \begin{warning}
     ...
     issue a warning. In most cases, such extra semicolons have no
  ...
  refers to the box in row~\textsf{r} and column~\textsf{c}.%
  \footnote{In this manual, we normally refer to the entries of
    tableaux and diagrams as \textit{boxes}, rather than \textit{nodes} \aTableau
    defines named coordinates, or anchors, for all the boxes in a tableau.
  ...
  are available, depending on which tableaux convention you are using:
  ...
  \option{star style}.  By default, boxes marked with a~$*$ are given the
 ...
 Some \Tableau[align=bottom]{124,357,68} words
  ...
  \option[fill=\aTableauColour{Star}]{star style=style}[\TikZ-styling]
...
\end{example}
  ...
  giving \TikZ stye-keys inside square brackets \keyword{[...]}.
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
 ...
 horizontal, runner using:
  ...
  \option[ball color=\aTableauColour{Star}, text=\aTableauColour{Main}]{abacus star style=style}
     [\TikZ-styling]
  ...
  \TikZ-styling does not fully control the style of these features.
  ...
  \TeXLive2025. If \aTableau causes difficulties when compiling your file
  ...
  literature using the \aTableau package, with the aim being to show how to
  use the package by using real examples.
  ...
  next two pictures is that they give non-trivial examples of how to use
...
we need to draw a tableau, connecting it with other combinatorial data.
