...
% Modified: 09:06 Friday, 29 November 2024
% ----------------------------------------------------------------------
...
%  described in the following sections.
  ...
  Currently, \option{ribbons} and \option{snobs} are \textit{not}
  supported for multitableaux and multidiagrams.
  ...
  \LaTeX{} delimiter. Setting the \option{left delimiter} or
  \option{right delimiter} to nothing removes the
  delimiter.
  ...
  \begin{example}
   ...
   \Multidiagram[delimiters={]}{[}]{1^2|2} \par\bigskip
   \Multitableau[left delimiter=,
    right delimiter=] {123,45,67|{12}{14}}
  \end{example}

  \option[\textbackslash emptyset]{empty}[\LaTeX{}]

  The \option{empty} option determines what is printed when a component
  diagram in a multitableau or multidiagram is empty. By default the
  empty set symbol $\emptyset$ is printed.
  ...
  \begin{example}
    ...
    \Multitableau[ribbons={|(Y)12rc}]{ 123,45 | 67,89} \par\bigskip
  \end{example}

  \option[B]{name}[character]

  The \option{name} option is used to change the default prefix for the
  box node names. Unlike in a tableau or diagram, the box names in
  multitableaux and muiltidiagrams take the form
  \textsf{B-k-r-c}, where
     \textsf{k} is the component index,
     \textsf{r} is the row index, and
     \textsf{c} is the column index
  of the box.

  \option[]{rows}[decimal number]
  ...
  \option{separator}\textsf{=|} draws a straight line between the
  components, but any character can also be used.
  ...
  \begin{example}
    ...
    \Multidiagram[separator color=brown, separator=:]{ 3,2 | 2^2 }
  \end{example}

  \option[0.3]{separation}[decimal (distance in \unit{cm})]

  Use \option{separation} to change the distance between the separators
  and the tableaux.
  ...
  \begin{example}
    \Multitableau[skew={2,1|1}]{ 123,45 | 67,89} \par\bigskip
    \Multidiagram[skew={1|2,1}]{ 3,2 | 2^2 }
  \end{example}

  \option[]{tabloid}

  Use \option{tabloid} for multitabloid diagrams and tableaux.
  ...
  \option[0]{xoffsets}[command separated list of $x$-offsets]
  \option*[0]{yoffsets}[command separated list of $x$-offsets]
