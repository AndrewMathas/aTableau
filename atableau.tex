...
% Modified: 15:21 Friday, 17 January 2025
% ----------------------------------------------------------------------
...
\NewDocumentCommand\AddOptionLink{m}
{
   ...
   {
      \phantomsection\label{Key:\str_lowercase:n{#1}}% add a label for references
...
{%
  ...
  {%
    ...
    \IfNoValueTF{#5} { \AddOptionLink{#3} }{ \AddOptionLink{#3-#5} }
...
%  described in the following sections.
  ...
  because of the following \option{ukrainian} setting.
  ...
  just loops that add the entries of the tableau. These loops set
...
\end{example}
  ...
  \option[]{entries}[contents, first, hooks, last, residues]<multitableau>
  \option*[]{charge}[$|$-separated list of charges]<multitableau>
  \index{contents}
  \index{residues}
...
\Abacus[north,entries=betas]{2}{4^2,2,1}\qquad\Abacus[west,entries=betas]{2}{4^2,2,1}
 ...
 As one more example, \emph{beta numbers} might be displayed using:
  ...
  \option[]{bead style}[\TikZ styling]
  ...
  explicitly, if there are $e$ runners then the bead to row~$r$ and
  ...
  \option{beta numbers=false} will remove the requirement that beta
  numbers be used in the bead specifications.

  \option[]{dotted cols}[list of runner indices]<abacus>
  \option*[]{dotted rows}[list of row indices]<abacus>
  ...
  As with tableau, consecutive rows and columns treated together.
  ...
  are removed first, followed by the \option{dotted cols}<abacus>. The
  output may need to be adjusted if the dotted rows and columns overlap.
  For example, consider the abacus:
  ...
  \begin{example}[lefthand width=0.16\textwidth]
  \Abacus[dotted cols={2,3}, dotted rows={2,3},
    tikz after = {
      \draw[aTableau/clearBoxes](A-1-1.south east)rectangle(A-4-4.north west);
      \draw[aTableau/dotted](A-1-1.south east)--(A-4-4.north west);
    }
  ]{6}{26,21^2,5^2,3,2^2,0^2}
  \end{example}

  \option[]{entries}[betas, residues, rows, shape]<abacus>

  Like the \option{entries} key for tableaux, the \option{entries} option
  for abacuses provides a shortcut for labelling the abacus beads
  by some common choices. The possible choices are:

  \begin{itemize}

    \item\option{entries=betas}\index{entries!beta}
    \label{beta numbers!abacuses}
    Label the beads by the corresponding \emph{beta numbers}.
  \begin{example}
  \Abacus[entries=betas]{3}{3,2^2,0^2} \qquad
  \Abacus[entries=betas]{4}{3,2^2,0^2}
  \end{example}
  ...
  type. You can can override this by setting \option{e} manually.
  \begin{example}
  ...
  For example, after \option{name=X} the node names take the
  form~$(\mathsf{X{-}r{-}c})$.
  ...
  example shows, rather than using \option{no shape} as an \aTableau-key
  you can use it directly as a \TikZ-style in \option{bead style}.
  (We include the \option{no shade} option partly for convenience and
  partly to advertise how it can be used.)
