...
% Modified: 00:52 Wednesday, 23 July 2025
% ----------------------------------------------------------------------
...
\begin{document}
  ...
  \end{example}
  As this example suggests, partitions can either be typed as a
  ...
  example, it is not possible to use polar coordinates.}
  are optional, being necessary if (and only if) the diagram is part of
  a~\keyword{tikzpicture} environment. See \autoref{Ex:coords} below,
  and \autoref{SS:coordinates} and \autoref{SS:abcoordinates}, for more
  ...
  being drawn. A list of the \aTableau commands and their options is
  ...
  \TikZ-styling can be applied to most of the components of the
  pictures produced by \aTableau commands.  The top of the last example
  defines two \TikZ-styles, \textsf{B} and \text{G}, which are used to add the
  \textsf{b}lue and \textsf{g}reen coloured hatchings to the two
  hooks. Inside the \keyword{tikzpicture} environment, the Young diagram
  ...
  A second important feature of \autoref{Ex:coords} is that it uses four
  ...
  \keyword{\RibbonTableau}, are derived from the \keyword{\Tableau}
  command.
  ...
  \section{Tableaux and Young diagrams}
    ...
    with sequences of integers, it is often more useful to visualise
    partitions as Young \emph{diagrams}, which are arrays of
    \emph{boxes}, or \emph{nodes}, in the plane%
    ...
    \emph{Australian} conventions are given by appropriate rotations of
  ...
  % --------------------------------------------------------------
    ...
    \begin{options}
      ...
      \item[*options] Optional arguments, or keys, which are described below, with many examples.
  ...
  \option{text entries} key, described below.
  ...
  Again, we emphasise that it is not necessary to add spaces between the
  ...
  \footnote{In particular, even though commas a used to separate the
  rows of the tableau, you do not need to enclose the style
  specifications inside both square brackets and braces, [\{...\}],
  because \aTableau is smart enough to look for the style \textit{inside} the
  square brackets.}
  ...
  The \option{label} keys in this example are to help you match each
  tableau with the corresponding command.
 ...
 Some \Tableau[align=bottom]{124,357,68} words
  ...
  \option{no border} is equivalent to \option{no border=true}.
  ...
  The \option{box fill} cannot be used in the same picture as
  the`\option{colours} key, which applies a list of fill colours using a
  residue pattern.
  ...
  only when you are using \option{text entries} that font commands like
  ...
  \end{itemize}
  This produces a colouring pattern consistent with Type~$A$ residues used in representation theory.
  ...
  By default, the type~$A$ residue pattern is used. Other residue
  patterns can be used by setting the \option{cartan} type, in which
  case you will also need to manually set the quantum characteristic
  \option{e} \textit{after} specifying the list of~\option{colours}.

  \begin{example}
  \Diagram[colours={yellow,white,white,yellow},
      entries=residues, cartan=C, e=3] {8,5,4}
  \end{example}
  ...
  how these pictures are drawn.

  \begin{example}
  \Tableau[cover={4^2,3}, cover boxes]{123,45,67} \qquad
  \Tableau[skew={2,1}, skew boxes]{123,45,67}
  \end{example}
  ...
  Use \option{inner wall=none} to remove the inner tableau walls. Alternatively, use \option{no boxes}.
  ...
  The \option{rotate} key rotates the tableau through the specified
  number of \textit{degrees} in an anti-clockwise direction.
  ...
  one \option{rotate} key should be used in any given picture. The
  different tableau conventions, such as \option{ukrainian} and
  \option{australian}, should be used in preference to the
  \option{rotate} key.
  ...
  \begin{warning}
    ...
    affect all \aTableau diagrams, including abacuses. It is better to
    use \keyword{\aTabset} to set the \option{box height} and
    \option{box width} sizes if you only want to change the default sizes of the
    diagrams and tableaux in your document.
  \end{warning}
  ...
  \keyword{\shortminus} command if it does not already exist, in which
  case it is defined using the command
  \verb|\DeclareMathSymbol\shortminus{\mathbin}{AMSa}{"39}|.}.
  In the second diagram above, \aTableau uses~\keyword{\shortminus 1}
  instead of~\keyword{-1}, which look like~$\shortminus1$ and~$-1$,
  respectively.\indexcmd{shortminus}
  ...
  \Diagram[entries=residues, e=3, charge=1,
  ...
  \keyword{\Tableau} command can be used with the ribbon command, so we
  ...
  \begin{itemize}[nosep]
    ...
    if they are  greater than $9$, or less than $0$.) The remaining
    entries in the ribbon are specified by a sequence of \textsf{r}'s
    and \textsf{c}'s, depending on whether the next box in the ribbon is
    given by increasing the row index, or decreasing the column index,
    respectively.
...
\end{example}
  ...
  \footnote{Out of the box, \TikZ \emph{does not} provide a way to disable
  ...
  is available whpartition the \aTableau package is loaded.}.
  ...
  symbol~$|$. The number of components in the quotients must be less
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
  ...
  \begin{quote}
    ...
    \begin{tabular}{Oll} \toprule
      ...
      $-$    & dash         & Draw a line that extends slightly past the first and last runners\\
      \_\!\_ & underscore   & Draw a line between the first and last runners\\
 ...
 horizontal, runner using:
  ...
  \option[0.1]{abacus top}[a decimal number]
  \option*[0.1]{abacus bottom}[a decimal number]

  The \option{abacus top} key controls the distance between the ``top''
  end of the abacus runners and the first row of beads, and
  \option{abacus bottom} controls the distance between the ``bottom'' of
  the abacus runners and the last row of beads  on the abacus. Both of
  these keys are interpreted as offsets to the row index, so
  decreasing \option{abacus top} raises the line at the ``top'' of abacus
  and increasing \option{abacus top} lowers the line at the ``bottom'' of
  the abacus.\footnote{Throughout this section, the word ``top'' and ``bottom''
  are correct when using the \option{south} convention for
  abacuses.  These words need to be appropriately modified when using
  the other abacus conventions.}

  \begin{example}
  \Abacus[abacus ends=--]{3}{2,1^2,0^3}\qquad
  \Abacus[abacus ends=--, abacus top=0, abacus bottom=1]{3}{2,1^2,0^3}
  \end{example}
  ...
  \begin{warning}
    The \option{no shade} key is \textit{not} a boolean key. In
  ...
  style abacus, which is commonly used in the literature for algebraic
  combinatorics and representation theory. When the \option{traditional}
  key is used the \keyword{\Abacus} commands draws a more
  traditional-style abacus, with runners look like rods and beads like
  ...
  appealing. Not all of the abacus options have been tested on
  traditional abacuses, so please
  \href{https://github.com/AndrewMathas/aTableau/issues}{report any problems}.

  \option[0.4]{bead height }[decimal number, height in \unit{cm}]
  \option*[0.4]{bead width}[decimal number, width in \unit{cm}]

  \begin{example}
  \Abacus[traditional, bead height=0.6]{3}{6,4,3,1,0} \qquad
  \Abacus[traditional, bead width=0.2]{3}{6,4,3,1,0}
  \end{example}

  \option[0.4]{beam height}[decimal number, height in \unit{cm}]

  \begin{example}
  \Abacus[traditional, beam height=0.2, bead width=0.2]{3}{6,4,3,1,0} \qquad
  \end{example}

  \option[0.4]{rod width}[decimal number, width in \unit{cm}]

  \begin{example}
  \Abacus[traditional, rod width=0.2, bead width=0.2]{3}{6,4,3,1,0} \qquad
  \end{example}
  ...
  Of course, you should replace $\langle$\keyword{\Command}$\rangle$ with whatever
  name you want to call your command. The $\langle$tableau command
  name$\rangle$ can be any of the \aTableau picture commands:
  \begin{center}
      \begin{tabular}{*5l}
         Abacus&
         Diagram&
      ...
      \end{tabular}
  \end{center}
  Finally the $\langle$keys$\rangle$ can be an arbitrary comma separated
  list of \aTableau keys.  Defining custom commands helps to ensure
  consistency throughout your document, and it also makes it easier to
  modify your ``design specifications'' because they will appear only in
  the definition of your custom command.
  ...
  \NewATableauCommand\MyTableau{Tableau}
    {french,border colour=magenta, box style={draw=orange,circle}}
  \MyTableau{123,45} \quad
  \MyTableau[box fill=red!20!white ]
    {123,[fill=orange,text=white]45}
  \end{example}
  ...
  \NewATableauCommand\MyDiagram{Diagram}
    {ukrainian, box fill=orange!20,shifted}
  \begin{tikzpicture}
    \MyDiagram(0,0){3,2}
  ...
  a risk that you may break some of the corresponding \aTableau
  commands. If you do change these styles directly, we strongly
  recommend that you \emph{append} to these styles rather than setting
  them directly.
  ...
  We do not recommend changing any of these styles directly because
  doing so can, potentially, break \aTableau functionality. In some
  cases, such as the tableau boxes, the \TikZ styling does not fully
  control the rendering of these features. Most of these styles can be
  changed using the key-value interface, which is another reason to
  avoid changing them by hand. Changes that you make to these styles are
  not guaranteed to be compatible with future releases of \aTableau.
  ...
  \option{ribbons} to add the $\alpha$ in row~$6$ and column~$8$.
...
\end{Example}
  ...
  \href{https://https://tex.stackexchange.com}{tex.stackexchange.com}. In particular, I thank
    Max Chernoff,
    Lars Madsen,
    Clea F. Rees,
  and
