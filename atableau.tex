...
% Modified: 01:46 Thursday, 02 January 2025
% ----------------------------------------------------------------------
...
%   \Diagram{2,1^2}\qquad
  ...
  This package grew out of my own research, so it does all of the things
  that I want it to do. On the other hand, it is written as flexibly as
  ...
  and abacuses are, then you probably don't need this package!) For more
  advanced usage, such as adding styling, I recommend skimming through
  the manual.
  ...
  The full list of commands provided by the \aTableau package is:
  \begin{center}
  ...
  (In addition, the \keyword{\aTabset} command can be used to set global
  \aTableau options.)

  Each of these commands is designed to be easy to use and easy to read,
  ...
  being drawn. The following options are common to all of the commands.
  ...
  you like using the \option{name} key. Similar anchors are created
...
%  described in the following sections.
  ...
  \option*[]{ribbon style}[\TikZ-styling commands]
  ...
  As a second example, we can use \keyword{tikzpicture={rotate=30}}
  ...
  \option[]{entries}[contents,first,hooks,last,residue]
  \index{contents}
  \index{first tableau}
  \index{hooks}
  \index{last tableau}
  ...
  \begin{example}
  % affine type C
  \Diagram[ukrainian,entries=residues, e=2, type=C]
    {8,4,2}
  \end{example}

  \begin{example}
  % twisted affine type A
  \Diagram[australian, entries=residues, e=2, type=AA]
    {8,4,2}
  \end{example}

  \begin{example}
  % twisted affine type D
  \Diagram[french, entries=residues, e=2, type=DD]
    {8,4,2}
  \end{example}
  ...
  are drawn with row~$r$ shifted by $(r-1)$-units along the row, so that the
  ...
  The definition of hook lengths for shifted diagrams can be found in the
  literature!
...
\end{example}
  ...
  can be specified using \option{type=??}.
  ...
  however, such diagrams can be be drawn using the \option{skew} key.
  In the same way, it is possible to draw multitableaux and
  multidiagrams that have components which are a mixture of non-skew,
  skew and shifted shapes.
