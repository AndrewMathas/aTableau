...
% Modified: 23:32 Wednesday, 01 October 2025
% ----------------------------------------------------------------------
...
\newcommand\TeXLive{\href{https://www.tug.org/texlive/}{TeXLive}\xspace}
...
\NewTColorBox[]{slide}{ O{} }
{
  ...
  boxed title style={sharp corners=uphill, colback=Azure},
  arc is curved,
  arc = 1.5mm,
  boxrule=0.6pt,
  ...
  colback=GhostWhite,
  colbacktitle=RoyalBlue!10!white,
  colframe=RoyalBlue!30,
  coltitle=NavyBlue,
  #1
}
...
\begin{document}
  ...
  \begin{itemize}[nosep]
    ...
    for contents in tableaux (see \options{entries=contents}).
  \end{itemize}
  ...
  ``locally'', for the current picture. As mentioned already, the
  ...
  \begin{options}
    ...
    \item[beamer] Set the \ctan{beamer} overlay command. See
    \autoref{SS:Beamer} for details.
 ...
 Some \Tableau[align=bottom]{124,357,68} words
  ...
  For other examples using node names and anchors see \autoref{Ex:ABnames},
  \autoref{Ex:pointing}, \autoref{Ex:waves} and \autoref{Ex:KLRW}.
  ...
  \begin{warning}
    ...
    \textit{scaled} environment of the form
    \begin{quote}
    ...
    Alternatively, you use \keyword{ransform canvas={scale=?}} in the
    \keyword{tikzpicture}.
  \end{warning}
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
 ...
 horizontal, runner using:
  ...
  Label each beads with the row index of the corresponding part in the
  partition.
  \begin{example}
  ...
  Label each bead with it the length of the corresponding row in the
  partition. That is, each bead is labelled by the
  ...
  \begin{warning}
    ...
    and column~\keyword{c}.  For abacuses, both the row and column
  ...
  \begin{warning}
    ...
    particular, the \option{no bead} key is \textit{not} an inverse key to
    \option{bead}.
  \end{warning}
  ...
  document class is being used, and it is not available otherwise.

  The \ctan{beamer }\textit{overlay specifications} are described in
  section 3.10 of it manual. The basic idea is that they allow you to
  specify those slides of a \ctan{beamer} presentation where particular
  content appears, or does not appear.

  \begin{Example}[text only, listing style=LaTeX]
    \lstinputlisting{atableau_beamer.tex}
  \begin{slide}[title=Slide 1]
    \includegraphics[width=0.98\textwidth,page=1]{atableau_beamer.pdf}
  \end{slide}
  \begin{slide}[title=Slide 2]
    \includegraphics[width=0.98\textwidth,page=2]{atableau_beamer.pdf}
  \end{slide}
  \begin{slide}[title=Slide 3]
    \includegraphics[width=0.98\textwidth,page=3]{atableau_beamer.pdf}
  \end{slide}
  \end{Example}
  ...
  extensions, rather than the \ctan{beamer} commands, is that the code
  is slightly easier to read and slightly easier to type.
  ...
  be changed using the \option{beamer} key. As an example, the following
  pairs of commands are equivalent.

  \begin{Example}[listing only]
  \Abacus<4,5>[beamer=uncover]{3,2}      \Tableau<2-4>[beamer=invisible]{12345,789}
  \uncover<4,5>{\Multidiagram{3,2|1^2}}  \invisble<2-4>{\Tabloid<2->{12345,789}}
  \end{Example}
  ...
  Options starting with \keyword{(no)} are a pair of boolean options and
  ...
  For the reasons explained in their
  \href{https://info.arxiv.org/help/faq/texlive.html}{Frequently Asked Questions},
  the \LaTeX\ distribution on the \arXiv is typically 1-2 years behind
  the latest \LaTeX\ release. The \aTableau package relies on recent
  advances in the \LaTeX3 programming environment. In particular, it
  requires \TeX Live 2024, or later to compile.

  As of October 2025, the \arXiv is working to update their \LaTeX\
  distribution to \TeXLive 2025. It will  not possible to compile
  \LaTeX\ files using the package \aTableau on the \arXiv until they do
  this. You can, however, email
  \href{mailto:arxivservice@gmail.com}{arxivservice@gmail.com} to
  request permission to upload a PDF file for your paper.
  ...
  \keyword{tikzpicture}. Two very similar \emph{pics} are used to add
  the up and down strings to this picture, taking as input the string
  ...
  next two pictures is that they give non-trivial examples of who to use
  the named nodes for the tableaux boxes; see \autoref{E:NodeNames}.
...
KLRW diagram~\cite{Bowman:ManyCellular}.

\begin{Example}[label=Ex:KLRW]
\begin{tikzpicture}[red string/.style={red, ultra thick},
  ...
  \Multidiagram(0,1.3) % the multipartition, filled with residues
   [scale=0.7, ukrainian, entries=residues, e=3, scale=1.4,
    rotate=4, no separators]{2^2,1|3,2,1}
  ...
  { % add solids, ghosts and lines from the tableau to the KLRW idempotent
    \draw[thin, DarkViolet](A-\k-\r-\c) -- (A-\k-\r-\c |- 0,0);
    \draw[solid](A-\k-\r-\c |- 0,0)--++(0,1);
