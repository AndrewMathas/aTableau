...
% Modified: 15:31 Thursday, 09 October 2025
% ----------------------------------------------------------------------
...
\NewDocumentCommand\SaveExample{m +m}
{
    ...
    \iow_now:Ne  \__aTableau_example_iow { Example~\theexample }
    \iow_now:Nn  \__aTableau_example_iow {}
...
\begin{document}
  ...
  \begin{example}[lefthand width=0.4\textwidth]{diagramUkrainian}
  ...
  picture. As mentioned already, the default options for the document
  can also be set via
  ...
  \option{border=true} and to \option{no border=false}. Similarly, the
  \option{no border} key is equivalent to \option{no border=true}, and
  to \option{border=false}.
  ...
  this example. (In fact, the right-hand picture could be drawn without
  using a \keyword{tikzpicture} environment by using the
  ...
  \begin{example}{diagramUkrainain}
  ...
  the default \option{abacus star}. As with the tableaux commands,
...
\begin{tikzpicture}[red string/.style={red, ultra thick},
  ...
  { % add solids, ghosts and lines from the tableau to the KLRW idempotent
    \draw[thin, brown](A-\k-\r-\c) -- (A-\k-\r-\c |- 0,0);
