...
% Modified: 15:30 Friday, 15 May 2026
% ----------------------------------------------------------------------
...
% aTableau meta data, which is defined in atableau.sty via the ini file
...
\begin{document}
      ...
      \begin{longtable}{lp{0.6\textwidth}l} \toprule % longtable allows footnotes
        ...
        \textcolor{aTableauMain}{Convention} &\textcolor{aTableauMain}{\aTableau command} & \textcolor{aTableauMain}{Picture} \\ \midrule
         \showrowcolors
          ...
          more common nomenclature from the literature. The \option{ukrainian} option
          is still supported.}
  ...
  and \text{G}, are defined. These styles are used to add the \textsf{b}lue and
  ...
  will not not actually change the picture.) The use of coordinates become more
  ...
  you can save little bit of typing using the \option{styles} key:
  ...
  In each case, use \option{charge} to offset the generated entries. The possible choices are:
  ...
  minus one inside a tableau s typeset using \keyword{\shortminus1},
  ...
  \Diagram[entries=rows]{3,2,2} \qquad
  \Diagram[french, entries=rows]{2^2,1}
  \Diagram[entries=rows, charge=2]{2^2,1}
  \end{example}
  \index{tableau!rows}
  ...
  used.  In many cases, the values of multitableaus and
  multidiagrams keys should be separated by the pipe symbol \keyword{|} to
  ...
  As this example shows, the partitions given to \option{cover} are not
  unchanged by \option{shifted} or \option{skew} are also used.
...
\end{Example}
  ...
  Most of the diagram is drawn using two \emph{pics} s,for the up and
  down string.  Pics are custom \TikZ styles, which are a good way of
  adding repeating features to a \keyword{tikzpicture}. To draw the up
  and down strings in this picture we define two very similar pics
  \textit{ustring} and \textit{dstring}, which take as input the string
  colour, the starting box, the list of the boxes that the string goes
  through, and the final box.
...
\aTabset{colour theme=classic, tikzpicture={>=Stealth},
  tick length=0.1,
  styles={
    semi={shape=semicircle,inner sep=0pt,minimum height=0.7mm},
    L={semi,bead=red,shape border rotate=270,anchor=west}, % left balls
    R={semi,shape border rotate=90,anchor=east},          % right balls
    Cap={out=30,in=150},    % curved arrows
    larrow={thick,red, <-}, % left arrow
    rarrow={thick,blue,->}, % right arrow
    landed={text=white},    % labelled ticks
  }
}
\Abacus[dotted cols={0,2,4,6,8,9,10,11,13,15,17,19},
  abacus ends=||, runner sep=0.6, bead sep=0.8, rows=9,
  tikz after={
    \draw[rarrow](A-4-1.base)node[below left=0.2]{$c_s$}
        to[Cap](A-4-5.base)node[below right]{$d_s$};
    \draw[rarrow](A-4-12.base)node[below left]{$c_2$}
        to[Cap](A-4-16.base)node[below right]{$d_2$};
    \draw[rarrow](A-6-5.base)node[below left]{$c_{s-1}$}to[out=30,in=180]+(2.2,0.5);
    \draw[rarrow](A-6-16.base)node[below left]{$c_{1}$}to[out=30,in=180]+(2.2,0.5);
    \draw[rarrow,<-](A-7-1.base)node[below left]{$c_{1}$}to[out=150,in=0]+(-2,0.5);
    \draw[rarrow,<-](A-5-12.base)node[below left]{$d_{3}$}to[out=150,in=0]+(-2,0.5);
    \draw[larrow,->](A-3-3.base)to[out=150,in=0]+(-2,0.5);
    \draw[larrow](A-0-14.base)to[Cap](A-0-18.base);
    \draw[larrow,->](A-1-14.base)to[out=150,in=0]+(-2,0.5);
    \draw[larrow](A-1-3.base)to[Cap](A-1-7.base);
    \draw[larrow](A-2-7.base)to[out=30, in=190]+(1,0.4);
    \draw[larrow](A-2-18.base)to[out=30, in=190]+(1,0.4);
}]{19}{|4_b,4,4||2,[L]2,[R]1||6||4,[R]1,[L]1|||||4||2,[L]0,[R]0||6||4,[R]1,[L]0}
\end{Example}

The most interesting features of this example are the use of styles to change the
shape of some nodes into half-spheres, and the use of named coordinates
to draw the arrows via the \option{tikz after} key. Notice also that the
abacus is specified as a quotient (\autoref{SS:quotients}).
