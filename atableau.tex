...
% Modified: 09:42 Monday, 13 January 2025
% ----------------------------------------------------------------------
...
%   \Diagram{2,1^2}\qquad
  ...
  a~\keyword{tikzpicture} environment. See \autoref{SS:coordinates} for
  more detail about how these coordinates work.

  The key-value \textit{options}, which control the style and appearance
  of the final diagram, are optional.  The possible options, or keys,
  for the \aTableau commands are described in detail below, with
  examples. This section emphasises only some of these options together
  with the fact that every key can be set ``globally'' (inside the
  current \LaTeX\ group), by using \keyword{\aTabset{...options...}}. As
  discussed already, the default options for the document can also be
  set via \keyword{\usepackage[options]{atableau}}.\indexcmd{aTabset}
  ...
  \begin{options}
    ...
    after the diagram drawn by \aTableau.
...
%  described in the following sections.
  ...
  done by adding style prefixes to the tableau entries.  To
  take advantage of these styles you need to have at least rudimentary
  knowledge of \TikZ-styling. The examples in this manual give a good
  ...
  \emph{ribbons} to a tableau.  The \option{ribbon style} key controls
  the default ribbon style. The \option{ribbon box} and \option{snob
  ribbon style} keys work in the same way as \option{path box} and
  \option{path box style}.

  The ribbon specifications are identical to those used for
  \option{paths}.  For more details see \autoref{S:RibbonTableaux},
  which describes the \keyword{\RibbonTableau} command.
  ...
  \option*[draw=\aTableauColour{Inner},thin]{snob style}[\TikZ-styling commands]
  \option*[-]{snob box}[A node entry]
  \option*[-]{snob box style}[\TikZ-styling commands]

  Use \option{snobs} to add ribbons to the tableau, which are
  placed \textit{after} the border of the diagram is drawn. (Writing
  \textit{ribbons} backwards gives \textit{snob}bir.) The
  \option{snob style} key controls the default style of snobs. The
  \option{snob box} and \option{snob box style} keys work in the same way as
  \option{path box} and \option{path box style}.
  ...
  The difference between this pictures and the one given for
  ...
  \option{snobs} and use \option{ribbons}.
  ...
  As described in \autoref{S:Tabloid}, each of these keys has associated
  keys to change the style of the ribbon, set a content of each box on
  the ribbon or path and a key to set the style of these boxes. More
  explicitly, the keys \option{path style}, \option{path box} and
  \option{math box style} are associated to \option{paths}.
  ...
  \Diagram[ukrainian, inner style=dotted,
    ...
    path box=\bullet,
    paths={(R)15rccrc[b]rc,(blue)43[b]rcc}
  ]{5^2,3^3}
  \end{example}
...
\end{example}
  ...
  of~\cite{EvseevMathas:DeformedKLR}, rather than the somewhat misguided
  ...
  the component tableaux are put on top of each other.
  ...
  entries and then \option{snobs=...} is used to rewrite the entries in
  the Garnir belt.
  ...
  \Tableau[ukrainian, box font=\scriptsize,
     tikz after = {\draw[ultra thick,teal]
