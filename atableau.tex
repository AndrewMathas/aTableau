...
% Modified: 12:25 Sunday, 22 June 2025
% ----------------------------------------------------------------------
...
\newenvironment{warning}{\list{\raisebox{-2ex}{\textcolor{red}{\textdbend}}\quad}{\leftmargin=3em}\item}{\endlist}
...
\begin{document}
  ...
  pictures found in the literature.\footnote{In this manual,
  ``diagram'' usually means ``Young diagram'', so a ``picture'' is
  anything drawn by an \aTableau command.}
  ...
  As detailed below, it is possible to apply \TikZ-styling to the different
  ...
  notation, which is usually shorter and easier to (proof)read.
  ...
  \aTableau options, or keys, which are described below.  For example,
  to make the tableaux and Young diagrams commands use the
  ...
  of key-value options that modify and embellish the picture drawn by the command. In addition, optional
  ...
  document, or it can be incorporated into a larger
  \keyword{tikzpicture} environment, making it possible to use
  \aTableau commands to create more complicated pictures.
  ...
  a~\keyword{tikzpicture} environment. See the example below, and \autoref{SS:coordinates} and
  \autoref{SS:abcoordinates}, for more detail about using these coordinates
  ...
  ``locally'', for the current picture. As discussed already, the
  ...
  being drawn. A list of the \aTableau commands and their options
  is given in \autoref{S:keys}, with the following sections giving more
  details and many examples. The following keys\footnote{Each key name is a
  hyperlink to the corresponding section of the manual.} are common to all
  \aTableau commands:
  ...
  \begin{options}
    ...
    the colours of the \aTableau pictures using the key-value interface.
  ...
  and \option{no border} keys are inverse keys. Using the
  \option{border} key is equivalent to \option{border=true} and to
  \option{no border=false}, and the \option{no border} key is equivalent
  ...
  \begin{warning}
     ...
     visible effect, but it is still best to omit it to avoid potential
     issues.
  \end{warning}
  ...
  environment since it was given the Cartesian coordinate $(0,0)$. A
  ...
  diagram, where (\textsf{A-r-c}) refers to the box in row~\textsf{r} and
  ...
  \begin{warning}
    ...
    requires an up-to-date version of \LaTeX. You may need to update
  ...
  \footnote{As shown below, a better approach is to define a
  \TikZ-style whenever you are using ``complicated'' \TikZ-styles.}
  This is necessary to ensure that  \LaTeX\ does not get confused by the
  commas in the style setting (because commas are used to separate
  ...
  By default, the colours are applied in type~$A$ residue order. This
  means that, when using the \option{english} convention, colours are assigned
  cyclically:
  \begin{itemize}[nosep]
  ...
  \option{e} \textit{after} specifying the list of~\option{colours}. For
  more information see the description of \option{entries=residues}, and
  the next example.
  ...
  keys cannot be used in the same picture.
  ...
  \begin{warning}
    ...
    is better to use the \option{box height} and \option{box width} options.
  \end{warning}
  ...
  and superscripts. The package automatically rescales tableau when
  ...
  By default, the border of a ribbon tableau is automatically calculated
  ...
  As shown, partitions can be given using exponential notation.
...
\end{example}
  ...
  introduced the abacus, with beads and runners, as another way to
  represent a partition~\cite{James:YoungD}. The abacus is particularly
  useful in modular representation theory, where the number of runners
  is the (quantum) characteristic. Abacus combinatorics are now used in
  the representation theory of many algebras, with their use now
  extending far beyond the symmetric groups.
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
 ...
 horizontal, runner using:
  ...
  style abacus, which is commonly used in the combinatorics and
  representation theory literature. When the \option{traditional} key is
  enabled, \keyword{\Abacus} instead draws a more traditional-style
  abacus, with runners look like rods and beads like discs.
  ...
  could be done with a little less code. From the \aTableau
...
\end{Example}
  ...
  \keyword{\shortminus} command for shorter minus signs, which are used
  ...
  that are not intended to be applied to a given picture will be
  silently ignored.
  ...
  hope that one of these theme will appeal. The \option{colour theme}
  ...
  \autoref{Ex:DottedAbacus}), to use these styles in your own drawings.
  ...
  to avoid changing them by hand.
