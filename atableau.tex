...
% Modified: 23:14 Friday, 15 May 2026
% ----------------------------------------------------------------------
...
% aTableau meta data that is defined in atableau.sty via the ini file
\newcommand\aTableauVersion{\tl_use:N\l__aTableau_version_tl}
\newcommand\aTableauRelease{\l__aTableau_release_date_tl}
\newcommand\aTableauRepository{\l__aTableau_repository_tl}
...
\def\@oddfoot {\color{aTableauMain} Version \aTableauVersion~(\aTableauRelease)\hfill\thepage}
\def\@evenfoot{\color{aTableauMain} \thepage\hfill Version \aTableauVersion~(\aTableauRelease)}
...
% define the heading as a command because it contains \aTableauVersion
\newcommand\aTableauHeader{%
  \begin{center}
    \begin{tikzpicture}
      ...
      \node[anchor=east,boxes] at (\paperwidth-4cm,0) {Version \aTableauVersion};
    \end{tikzpicture}
  \end{center}
}
...
\hypersetup{
  ...
  pdfinfo={
    ...
    Subject ={ LaTeXing abacuses and tableaux },
    Title   ={ aTableau - \aTableauVersion }
  },
  colorlinks=true,
  linkcolor=blue,
  citecolor=FireBrick,
...
\begin{document}
      ...
      \begin{longtable}{lp{0.6\textwidth}l} \toprule % longtable allows footnotes
        ...
        \textcolor{aTableauMain}{Convention} &\textcolor{aTableauMain}{\aTableau command} & \textcolor{aTableauMain}{Picture} \\ \midrule
         \showrowcolors
          ...
          more common nomenclature from the literature. The
  ...
  inside \texttt{tikzpicture} environments, including polar coordinates
  ...
  \begin{options}
    ...
    Set \ctan{beamer} overlay command. See \autoref{SS:Beamer} for details.
  ...
  \begin{warning}
    If you use an \aTableau command inside a \keyword{tikzpicture}
  ...
  tableaux when they are used as subscripts and superscripts, with the
  ...
  (Notice the use of \keyword{\aTabset} here. Using \keyword{\tikzset{styles={...}}}
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
 ...
 \begin{example}{abacusEndsEast}
  ...
  Please
  \href{https://\aTableauRepository issues}{report any problems}.
...
\end{Example}
  ...
  try uploading the latest \aTableau style file,
  ...
  package to construct some non-trivial real-world examples.
  ...
  The ``hard work'' in drawing this picture diagram is done by defining
  two \emph{pics}s, \textit{ustring} and \textit{dstring}, for the up and
  down strings. Pics are custom \TikZ-styles that are a good way of
  adding repeating features to a \keyword{tikzpicture}. These pics take
...
tikz after={ % add the red and blue arrows
  \draw[barr](A-4-1) to[Cap](A-4-5);
  \draw[barr](A-4-12) to[Cap](A-4-16);
  \draw[barr](A-6-5)node[below left]{$c_{s-1}$}to[out=30,in=180]+(2.2,0.5);
  \draw[barr](A-6-16)to[out=30,in=180]+(2.2,0.5);
  \draw[barr,<-](A-7-1)to[out=150,in=0]+(-1,0.4);
  \draw[barr,<-](A-5-12)to[out=150,in=0]+(-1,0.4);
...
}]{19}{|4_{d_1},4_b,4,4_{c_s}||2,[R]2,[B]1||5,4_{d_s}||4,[B]1,[R]1|||||
         4_{d_s},4_{c_2}||2,[R]0,[B]0||5_{c_1},4_{d_2}||4,[B]1,[R]0}
\end{Example}

  The most interesting features of this example are the use of styles to change the
  shape of some abacus beads into half-spheres, and the use of named coordinates
  to draw the arrows between beads using the \option{tikz after} key. Notice also
  that the abacus is specified as a quotient (\autoref{SS:quotients}).
  ...
  page:
  \begin{center}
    \href{https://\aTableauRepository issues}{\aTableauRepository issues}.
  \end{center}
