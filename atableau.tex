...
% Modified: 11:19 Wednesday, 15 January 2025
% ----------------------------------------------------------------------
...
%  described in the following sections.
  ...
  \keyword{\matrix} command. The \keyword{\SkewTableau} commands are not
  explicit components in the \keyword{tikzpicture} environment, being
  all (implicitly) inside \TikZ-nodes.
  ...
  \begin{xltabular}{0.9\textwidth}{lXcc}
    ...
    \option{dotted cols}       & Specify columns to replace with dots                & \Yes & \Yes \\
    \option{dotted rows}       & Specify rows to replace with dots                   & \Yes & \Yes \\
    ...
    \option{styles}            & Short-hand for defining single use \TikZ-styles     & \Yes & \Yes \\
  ...
  of the tableau boxes. In particular, in the last example, \option{box
  width=0.2} has no effect because of the following
  \option{ukrainian} setting.

  \begin{example}
  \aTabset{align=top}
  \Tableau[box height=0.3]{123,455,674} \qquad
  \Tableau[ukrainian, box width=0.2]{123,455}
  \end{example}
  ...
  Here is a cute use of \option{paths} to add arrows to a ribbon
  tableau:
  ...
  between the two boxes on the left respectively, where left and right
  is with respect to order of the two boxes in the draw command.  The
  two \keyword{\foreach} statements might look complicated, but they are
  just loops which add the entries in the tableau. These loops set
  \keyword{\r}, \keyword{\c} and \keyword{\l} equal to the row index,
  column index and label, respectively, using the following sequences of
  triples.
  ...
  \option{snob box style} tha work in the same way. Many of the
...
\end{example}
  ...
  \footnote{Surprisingly, \TikZ \emph{does not} provide a way to disable
  a shading that is already in effect. The \emph{no shade} style is
  based on a \href{https://tex.stackexchange.com/a/85750/234252}{stack
  exchange post}. The \emph{no shade} key is available if the \aTableau
  package is loaded.}.
  ...
  In such cases, you probably want to replace the bead with a label, so
  you will also need to change the text, which is white by default.
  ...
  \begin{Example}
  \Abacus[south]{2}{4^3,2,1}\qquad \Abacus[east]{2}{4^3,2,1}\qquad
  \Abacus[north]{2}{4^3,2,1}\qquad \Abacus[west]{2}{4^3,2,1}
  \end{Example}

  The name of the convention gives the direction that the abacus grows in.
 ...
 As one more example, \emph{beta numbers} might be displayed using:
  ...
  abacus. As shown, use the \TikZ-style \keyword{ball color} to
  ...
  As with tableau, you can also change the size f the abacuses using
  \option{scale}, \option{xscale} and \option{yscale}. Changing these
  keys globally affects both abacuses and tableaux.
  ...
  \TikZ-styling to the default bead style.
  ...
  \begin{itemize}
    ...
    \item\option{entries=beta}\index{entries!beta}
  ...
  \item\option{entries=rows}\label{entries!rows}
  ...
  \begin{example}
    ...
    \Abacus[bead style={no shade, shape=diamond,
      fill=brown, text=white}]{3}{3_1,2_2,2_3,0^2}
  \end{example}

  As the first example shows, if you use the \option{no shade} key then
  you need to define a new style for the abacus beads. As the second example
  ...
  \option[-]{rows}{non-negative integer}
  ...
  \option{rows} key to change the number of rows displayed. If you use
  the \option{rows} key, then it is your responsibility to ensure that
  ...
  \option*[0.1]{tick length}[a decimal number, length in \unit{cm}]
  ...
  ticks on the abacus runners. The other two key have he obvious meanings.
