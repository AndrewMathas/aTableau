...
% Modified: 15:04 Wednesday, 16 April 2025
% ----------------------------------------------------------------------
...
\begin{document}
  ...
  In addition, it is possible to apply \TikZ-styling to the different
  ...
  appearance of the \aTableau picture. The keys are optional and are
  ...
  details and many examples. The keys\footnote{Each key name is a
  hyperlink to the corresponding section of the manual.} that are common to all
  ...
  \begin{options}
    ...
    provides several other colour themes. It is also possible to customise
  ...
  \begin{warning}
     ...
     environment. If you accidentally add a semicolon, then \TikZ will
     give you a warning but, most of the time, it will not cause any
     harm.
  \end{warning}
  ...
  characters, or given by commands. In true \TeX-style, such entries can be added to a
  ...
  drawing of the tableau border. The \option{no border} key is inverse
  to \option{border}, so \option{no border} is equivalent to \option{no
  border=true}the drawing of the (thick) border wall around a tableau.
...
\end{example}
  ...
  Finally, we note that you can use the \option{traditional} to draw
  more traditionally looking abacus display:
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
 ...
 horizontal, runner using:
  ...
  By default, the \keyword{\Abacus} draws a ball-and-stick version of
  the abacus, which is a common way of drawing abacuses in papers in
  ...
  Chinese abacus, where the runners look or like rods and the beads are
  disc shaped. This section describes how to use the
  \option{traditional} key, together with a description of how it
  changes the default settings and the additional keys that only apply
  to traditional abacuses.

  \option[false]{traditional}[true/false]

  When using the \option{traditional} option, the \keyword{\Abacus}
  commands draw an abacus that looks more like  a traditional Chinese
  abacus, where the runners are drawn as rods and the beads are wider.
  ...
  \href{https://github.com/AndrewMathas/aTableau/}{report any problems}.

  \option[0,4]{bead height }[decimal number, width in \unit{cm}]
  \option*[0,4]{bead width}[decimal number, width in \unit{cm}]

  \option[0,4]{beam height}[decimal number, width in \unit{cm}]
  ...
  could be done a little more efficiently. From the \aTableau
...
\end{Example}
  ...
  \begin{xltabular}{0.9\textwidth}{l>{\raggedright}Xcc}\toprule
    ...
    \option{shape}             & Specify shape of ribbon tableau                     & \Yes & \No \\
  ...
  The colours in the pictures drawn by \aTableau are controlled by the six base colours:
  \begin{center}
    \begin{tabular}{*3l}
    aTableauMain      &
    aTableauInner     &
    ...
    \end{tabular}
  \end{center}
  Colours of individual components of the pictures using the keys
  described above. Creating a good colour scheme is difficult, and it is
  partly subjective, so \aTableau provides three different colour themes
  in the hope that one of them may appeal. The \option{colour theme} key
  ...
  involved. We do not recommend changing them directly because changing
  these styles can, potentially, break \aTableau functionality. In
