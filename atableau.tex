...
% Modified: 21:33 Sunday, 13 July 2025
% ----------------------------------------------------------------------
...
\begin{document}
  ...
  keys, their default values, and gives examples of their usage.
 ...
 Some \Tableau[align=bottom]{124,357,68} words
  ...
  By default, the colours are applied sing the type~$A$ residue pattern.
  ...
  the tableau entries typeset as either in mathematics mode or text mode,
  ...
  \begin{warning}
    ...
    \option{box width} sizes if you want to change the default sizes of the
  ...
  Tableaux and diagrams are rarely, but occasionally, used as subscripts
  or superscripts. The package automatically rescales tableau when they
  are used as subscripts and superscripts, with the \option{script} and
  \option{scriptscript} keys giving finer control over the sizes of
  subscripts and subsubscripts, respectively. The values of the
  \option{scale}, \option{xscale} and \option{yscale} keys are taken
  into account by these keys.
  ...
  Equivalently, the commands \keyword{\SkewTableau{2}{123,45}} and
  \keyword{\SkewDiagram{2^2,1}{3,2^2}}, respectively, can be used for
  these two pictures. See \autoref{S:Skew} for the options specific to
  skew tableaux and skew diagrams.
  ...
  environments can reference the node names in the last tableau, which
  ...
  \begin{itemize}[nosep]
    ...
    \item The key border, when by \option{skew border}, and the border
    of the tableau are drawn.
  ...
  values\cprotect\footnote{If not already defined, \aTableau defines
  \keyword{\shortminus} using
  ...
  A partition $\mu$ is \emph{contained} in another partition $\lambda$,
  written as~$\mu\subseteq\lambda$, if $\mu_k\le\lambda_k$, for
  ...
  the \emph{inner shape} and $\lambda/\mu$ is the \emph{outer shape}. A
  ...
  be given as a comma-separated list of non-negative integers, or in
...
\end{example}
  ...
  \begin{quote}
    \keyword{\NewDiagram \Command { atableau settings }} \newline
    \keyword{\NewTableau \Command { atableau settings }}
  \end{quote}

  Of course, you should replace ``\keyword{\Command}'' with whatever
  name you want to call your command. Defining a custom command helps to
  ensure consistency throughout your document, and it also makes it
  easier to modify the ``design specifications'' because they appear
  in just one place.

  The \keyword{\NewDiagram} command creates custom diagrams, and the
  \keyword{\NewTableau} command creates custom tableaux. The commands
  created this way can be used in exactly the same ways as the \aTableau
  picture commands. In particular, they can be used inside and outside
  \keyword{tikzpicture} environments, they accept \aTableau
  key-value settings as an optional argument, and tableau entries can be
  styled.

  \begin{example}
  \NewTableau\MyTableau{french,border colour=magenta,
  ...
  \begin{example}
  \NewDiagram\MyDiagram{ukrainian,
    box fill=orange!20,shifted}
  \begin{tikzpicture}
    \MyDiagram(0,0){3,2}
  ...
  These two commands are mainly provided for convenience because it is
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
 ...
 horizontal, runner using:
  ...
  literature. The aim of this section is to show how these pictures can
  be drawn using the \aTableau package.
...
\end{Example}
  ...
  \begin{enumerate}
    ...
    \item \keyword{\NewTableau} for defining custom tableau commands;
    see \autoref{SS:NewTableau}
    \item \keyword{\NewDiagram} for defining custom diagram commands;
    see \autoref{SS:NewTableau}
  ...
  involved. We do not recommend changing any of these styles directly
  because doing so can, potentially, break \aTableau functionality. In
  ...
  can be changed using the key-value interface, which is another reason
  to avoid changing them by hand. Changes that you make to these styles
  are not guaranteed to be compatible with future releases of \aTableau.
  ...
  Several features in \aTableau have benefited from discussions on
