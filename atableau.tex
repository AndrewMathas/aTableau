...
% Modified: 16:17 Sunday, 13 July 2025
% ----------------------------------------------------------------------
...
\begin{document}
  ...
  hyperlink to the corresponding section of the manual.}
  ...
  command, as shown by the left-hand picture, even though most of the
  commands in the example are \TikZ commands. In particular, the shading
  of the boxes in the \emph{$(1,2)$-hook} and the \emph{$(1,2)$-rim
  hook} is done by the \option{ribbons} key. (In fact, the right-hand
  picture could be drawn without using a \keyword{tikzpicture}
  environment by using the \option{tikz after} key to add the three
  ...
  \begin{warning}
    The \aTableau package relies heavily recent advances in
    \ctan{LaTeX3}, so it requires an up-to-date version of \LaTeX. You
    may need to update your \LaTeX\ installation if it is older than
    \TeX Live 2024.
  \end{warning}
 ...
 Some \Tableau[align=bottom]{124,357,68} words
  ...
  anything that you like.  This feature is most useful when you
  ...
  braces when you want to use them as entries in a tableau:
  ...
  we highlight the more interesting ones, together with some new,
  ...
  values\cprotect\footnote{If not defined, \aTableau defines
  ...
  partition that contains all of its ribbons (the border can be
  specified with the \option{shape} key).  In the last example, the
  \option{no border} key is used to disable the ribbon tableau border to
  highlight ribbon, which is the full diagram for these two tableaux.
  ...
  parentheses $(...)$ at the \textit{beginning} of the ribbon
  specification. Unlike the other style specifications, the
  $*$-shorthand for the \option{star style} cannot be applied to an
  entire ribbon.
  ...
  The styles for a ribbon, which are given inside parentheses
  \keyword{(...)}, are applied to the region bounded by  the closed path
  given by the border of the ribbon. The ribbon style is applied first,
  after which any boxes in ribbon are drawn.  As the orange and yellow
  ribbons in example show, the style of any box in the ribbon takes
  precedence over the style of the full ribbon.

  It is often useful to label some of the boxes in a ribbon, which can
  be done by giving the box contents as a \textit{subscript} to the
  corresponding entry in the ribbon specification:
  ...
  below, this allows you to place boxes outside of the ribbon tableau.
...
\end{example}
  ...
  Sometimes, it is necessary to use multiple tableau styles within the
  same document. While it is possible to apply the required key-value
  ...
  name you want to call your command.
  ...
  \begin{example}
  \NewTableau\MyTab{french,border colour=magenta,
    box style={draw=orange,circle}}
  \MyTab{123,45} \quad
  \MyTab[box fill=red!20!white ]
  ...
  The \keyword{\NewDiagram} command is used in exactly the same way.
  ...
  Finally, we note that you can use the \option{traditional} key to draw
  a more traditional abacus display:
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
 ...
 horizontal, runner using:
  ...
  \subsection{Traditional abacuses}
...
\end{Example}
  ...
  default \aTableau values for the \aTableau options and the
  \keyword{\NewTableau} and \keyword{\NewDiagram} commands, for creating
  custom picture commands; see \autoref{SS:NewTableau}. (Finally, the
  package provides the \keyword{\shortminus} command for shorter minus
  signs, which are used for contents in tableaux.)
  ...
  dialect of English are tolerated.
