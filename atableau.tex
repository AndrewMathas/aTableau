...
% Modified: 11:59 Tuesday, 03 December 2024
% ----------------------------------------------------------------------
\documentclass{amsart} \usepackage[svgnames]{xcolor}
\usepackage{atableau} \usepackage{xspace} \usepackage{manfnt}
...
{%
  ...
  {%
    ...
    \textsf{\color{aTableauInner}\l_tmpa_tl}%
    \index{\l_tmpa_tl}%
    \seq_if_empty:NF \l_tmpa_seq%
    {
      \textsf{=}%
      \seq_pop_left:NN \l_tmpa_seq \l_tmpa_tl%
      \textsf{\color{orange}\l_tmpa_tl}%
    }%
  }
  {%
    \IfBooleanTF{#1}{\newline}{\bigskip}%
    \noindent\textsf{\textcolor{aTableauInner}{#3}}%
...
%   \Diagram{2,1^2}\qquad
  ...
  \keyword{tikzpicture} environment. To use the package, add the
  ...
  \option{French} convention, by default, you might use:
  ...
  \begin{center}
    \begin{tabular}{Lll}\toprule
      \multicolumn1l{\large\sffamily\color{MidnightBlue}Command}
                     & \large\sffamily\color{MidnightBlue} Diagram
  ...
  \begin{options}
    ...
    environment.  Use \option{tikzpicture=keys} to set the optional
  ...
  With boolean options, which are set to \textsf{true} or \textsf{false},
  ...
  example is that the \keyword{\Diagram} command is part of the
  ...
  column \textsf{c}. In the name (\textsf{B-r-c}), $r$ and $c$ should be
  replaced by the row and column index of a box in the tableau, because
  these anchors are \textit{only} defined for boxes in the tableau.
...
%  described in the following sections.
  ...
  example, notice the use of \option{name=A} to make the named
  ...
  for styling the individual boxes in a tableau. Using a key-value syntax,
  you can change aspects of the tableaux produced by
  \keyword{\Tableau} commands.  The rest of this section describes these
  ...
  the drawing of the border wall around a tableau. As with other boolean
  keys, \option{border} is equivalent to \option{border=true} and
  \option{no border=flase}. The \option{no border} key is inverse to
  \option{border}, so \option{no border} is equivalent to
  \option{no border=true}, and to \option{border=false}.
  ...
  prefix for the node names. See \autoref{E:NodeNames} for a description
  of the anchor names. Examples are given in \autoref{Ex:ABnames} and
  \autoref{Ex:pointing}.

  See the introduction to this section for examples and for more
  information about using node names and anchors.

  \option[]{ribbons}[list of ribbon specifications]
  \option*[]{snobs}[list of ribbon specifications]
  ...
  Use the \option{tabloid} for tabloids. This option is discussed in
  ...
  should be \textit{connected}. (For disconnected tableaux and diagrams, see
  ...
  All of the tableau and Young diagram commands use the same drawing
  order.
  ...
  The characters  \keyword{[]}, \keyword{,}, and \keyword{*} have
  special meanings in the \textit{tableau specifications}. Enclose these characters in
  braces when you want them to be entries in a tableau:
  ...
  becomes special when it is preceded by a matching~\keyword{[}
  character.
  ...
  weakly decreasing nonnegative integers, or by using \emph{exponential
  notation}:
  \index{exponential notation}
  \index{partition!exponential notation}
  ...
  \keyword{\Tableau} command can be used with \keyword{\Diagram}. We do
  not repeat all of the \keyword{\Tableau} options in this section.
  Instead, we highlight some of the more interesting options together
  with some new, diagram specific options.
  ...
  \option[]{show}[contents,first,hooks,last,residue]
  \index{contents}
  \index{first tableau}
  \index{hooks}
  \index{last tableau}
  ...
  \index{tableau!hooks}
  \index{tableau!last}
  \index{tableau!residue}

  The \option{show} option provides a shortcut for drawing some common
  tableaux of a specified shape. The possible choices are:
  \begin{itemize}
    ...
    \item\option{show=last} print the \emph{last} \textbf{standard}
    ...
    \index{tableau!last}
    \begin{example}
    \Diagram[show=last]{3,2,2} \qquad
    \Diagram[australian,show=last]{2^2,1}
    \end{example}
    ...
    \item\option{show=first} print the \emph{first}
    ...
    \index{tableau!first}
    \begin{example}
    \Diagram[show=first]{3,2,2} \qquad
    \Diagram[ukrainian, show=first]{3^2,2}
    \end{example}
    ...
    By default, residue sequences for affine type A, or the symmetric group and
    friends, are given. In addition, residues in affine type C are also
    supported using \option{type=C}.

    \begin{example}
    \Diagram[ukrainian,show=residues, e=2, type=C]{6,4,2}
    \end{example}
  ...
  Since the \keyword{\SkewTableau} and \keyword{\SkewTableaua} commands
  ...
  These two options are inverses of each other.  When \option{skew
  border} is \textsf{true}, the border of the (inner) skew shape is
  drawn.
  ...
  \option*[thin,gray]{skew box style}[\TikZ-styling]

  The \option{skew boxes} key draws the inner walls in the inside the
  inner partition of a skew shape. Use \option{skew box style} to change
  the default shading of these boxes.

  \begin{example}
  \SkewTableau[skew boxes] {2,1^2}{123,45,67}
  \SkewTableau[skew boxes, skew box style=dashed] {2,1^2}{123,45,67}
  \end{example}

  Using the options \option{skew border} and the \option{skew boxes}
  together gives the skew (inner) shape both a inner and outer borders.

  \begin{example}
  \SkewTableau[skew border, skew boxes] {2^2,1}{123,45,67}
  \end{example}

  \Todo{Fix the inner wall colour for the skew boxes}
  ...
  tableaux and diagrams: the \emph{shifted hook length is:
\[
   h_{ij}^* = \lambda_i + \begin{cases*}
       \lambda_j & if $j<\ell(\lambda)$,\\
       \max\set{k | \lambda_k\ge j+1-k\ge 1} & othewise.
   \end{cases*}
\]
  }.}
  ...
  \ShiftedDiagram[show=last]{4,2,1} \qquad
  \end{example}
  ...
  \option[]{ribbons}[comma separated ribbon specifications]\newline
  \option[]{snobs}[comma separated ribbon specifications]
  ...
  \option[draw=\aTableauColour{Inner}, thin]{ribbon style}{\TikZ-styling}
  ...
  ribbons, respectively. By default, ribbons have a inner wall and the
  boxes in ribbons have no styling.
  ...
  \subsubsection{Some ribbed examples}
  ...
  diagrams, respectively. These diagrams appear in many places in representation
...
\end{example}
  ...
  notation for partitions. The entries in a \keyword{\Multitableau} can
  ...
  The \option{conjugate} option prints the conjugate mutlitableaux and
  multidiagrams. Conjugation for multitableaux and multidiagrams reverses
  the order of the components and then conjugates all of the component
  diagrams.

  \begin{example}
  \Multitableau[conjugate]{ 123,45 | 67,89} \par\bigskip
  ...
  \Multidiagram[left delimiter=\langle,
      right delimiter={]}]{2,1|1} \qquad
  \Multidiagram[delimiters={\|}{\}} ]{1^2|2}
  \end{example}

  To remove a delimiter, set \option{left delimiter} or \option{right
  delimiter} to be empty.
  ...
  \option[\textbackslash textendash]{empty}[any \LaTeX{}]
  ...
  and multidiagrams the names of the take the form \textsf{B-k-r-c},
  where~\textsf{k} is the component index,~\textsf{r} is the row index,
  and~\textsf{c} is the column index of the box. The \textsf{B} can be
  changed to anything that you like using the \option{name} key.
  ...
  \option[]{ribbons}[a $|$-separated list of ribbon specifications]
  \option*[]{snobs}[a $|$-separated list of ribbon specifications]

  The \option{ribbons} and \option{snobs} keys work in exactly the
  same way for multitableaux and multidiagrams as they do for tableaux
  and diagrams except that the ribbons for different components are
  separated by a pipe $|$.

  \begin{example}
  \tikzset{R/.style={draw=red, thick, opacity=0.5},
           Y/.style={fill=yellow!20,opacity=0.5}}
  \Multitableau[ribbons={(R)11*r|(Y)12rc}]
      { 123,45 | 67,89}
  \end{example}

  \option[]{rows}[a decimal number]

  The \option{rows} option sets the number of rows covered by the
  delimiters in multitableaux and multidiagram.  By default, the
  \keyword{\Multitableau} and \keyword{\Multidiagram} commands choose
  the size of the delimiters to match the component diagrams, which may
  not be what you want especially when using the \option{australia} and
  \option{ukrainian} conventions. The value of \option{rows} can be a
  decimal number.

  \begin{example}
  \Multidiagram[ukrainian,scale=0.5]{3,2^2|...|2,1^2} \par\bigskip
  \end{example}
  ...
  \begin{example}
  \Multidiagram[no separators,scale=0.5]
  ...
  \option[\aTableauColour{Main}]{separator colour}[\LaTeX{} colour]
  ...
  \option[0.3]{separation}[a decimal number (the distance in \unit{cm})]
  ...
  \option[]{show}[contents,first,hooks,last,residues]
  \index{contents}
  \index{residues}
  ...
  \begin{itemize}
    ...
    \item\option{show=last} print the \emph{last} \textbf{standard} tableau of this
    shape, which has the numbers $1,2\dots,n$ entered in order from top
    to bottom down successive columns, and left to right along components (with appropriate modifications for
    ...
    \item\option{show=first} print the \emph{first} \textbf{standard} tableau of this
    shape, which has the numbers $1,2\dots,n$ entered in order from top
  ...
  \option[]{skew}{a $|$-separated list of partitions}
  ...
  \begin{example}
    ...
    \Multidiagram[skew={1|2,1}, skew border]{ 3,2 | 2^2 }
  \end{example}

  There is no command for drawing shifted multitableau or multidiagrams,
  however, they can be also be drawn using \option{skew}.  In the same
  way, it is possible to draw multitableaux and multidiagrams that have
  components that are a mixture of non-skew, skew and shifted shapes.
  ...
  \option[0]{xoffsets}[comma separated list of $x$-offsets]
  \option*[0]{yoffsets}[comma separated list of $y$-offsets]
  ...
  aligned, subject to the value of \option{separation}. Use the
  \option{xoffsets} to offset the $x$-coordinates of the component
  diagrams and \option{yoffsets} to offset their $y$-coordinates.
  ...
  A ``traditional Chinese abacus'', with beads and runners, gives another
  representation of a partition, which is particularly useful in modular
  representation theory. The partition corresponding to an abacus is
  given by counting the number of empty bead positions before each bead
  on the abacus. Use the \keyword{\Abacus} command to draw an abacus,
  using a similar syntax to the tableau and diagram commands.

  \keyword{\Abacus (x,y) [options] {number of runners} {bead specifications}}

  As with other \aTableau commands, the $(x,y)$-coordinates are
  necessary if and only if the abacus is part of a \keyword{tikzpicture}
  environment.  The number of runners is a positive integer that
  specifies the number of abacus runners. In their simplest form,
  the bead specifications are a comma separated list of non-negative
  integers specifying a partition, with exponential notation and
  optional styling and bead labels being supported.

  \begin{example}
     \Abacus{2}{5,4,1,1} \qquad
     \Abacus[rows=4]{3}{5,*4,1^2} \qquad
     \Abacus{4}{5,4,1^2,[ball color=teal]0^5}
  \end{example}

  \subsection{Bead specifications}

  \subsection{Abacus options}
