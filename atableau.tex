...
% Modified: 19:05 Tuesday, 14 January 2025
% ----------------------------------------------------------------------
...
%   style,
  ...
  moredelim=[s][keywordstyle2]{text entries}{=},
  moredelim=[s][keywordstyle2]{math entries}{=},
...
%   \Diagram{2,1^2}\qquad
  ...
  the \keyword{\aTabset} command, and you can change it for particular
  diagram.
  \indexcmd{aTabset}
  ...
  \begin{options}
    ...
    \item[math entries, test entries]\index{math entries}\index{text entries}
  ...
  American spelling variations of keys are tolerated. For example, you
  can use \option{center} and \option{color} instead of the correct
  spellings of \option{centre} and \option{colour}, respectively.

  \begin{warning}
    Unlike the egalitarian \aTableau settings, \TikZ-style settings
    \emph{do not} countenance English spelling!
  \end{warning}
  ...
  by the \option{ribbons} key. (In fact, this entire picture can be
...
%  described in the following sections.
  ...
  % --------------------------------------------------------------
  \section{Complete list of \aTableau commands and options}\label{S:keys}
  \rowcolors{2}{aTableauSkewFill}{}

  For this impatient, this section compactly lists the \aTableau
  commands and their options. Later section show how these are used,
  with examples.
  ...
  abacuses~--- or both!  Options starting with \keyword{(no)} are
  boolean options and their inverses. If not given a true or false value
  then they are implicitly set to true. Although not listed, spelling
  variations of keys from the American-English dialect are tolerated.

  \begin{xltabular}{0.9\textwidth}{lXcc}
    \toprule
    ...
    \option{cartan}            & Set Cartan type for residues                        & \Yes & \Yes \\
    ...
    \option{dotted cols}       & Specify diagram columns to replace with dots        & \Yes & \Yes \\
    \option{dotted rows}       & Specify diagram rows to replace with dots           & \Yes & \Yes \\
    ...
    \option{math entries}      & Typeset box and label entries in math-mode          & \Yes & \Yes \\
    \option{name}              & Prefix of named nodes                               & \Yes & \Yes \\
    \option{north}             & Make abacus grow in the northerly direction         & \Yes & \No \\
    ...
    \option{ribbon style}      & Sets style of a ribbon                              & \Yes & \No \\
    ...
    \option{text entries}      & Typeset box and label entries in text-mode          & \Yes & \Yes \\
    \option{tick}              & Set the colour of the abacus runner ticks           & \No  & \Yes \\
    \option{tick width}        & Set the width of the abacus runners                 & \No  & \Yes \\
    \option{tikz after}        & \TikZ-code injected after \aTableau picture         & \Yes & \Yes \\
    \option{tikz before}       & \TikZ-code injected before \aTableau picture        & \Yes & \Yes \\
    ...
    \option{valign}            & Set vertical alignment                              & \Yes & \Yes \\
    ...
    \option{xscale}            & Set \aTableau $x$-scale                             & \Yes & \Yes \\
    \option{yoffsets}          & Set $y$-offsets for components in a multitableau    & \Yes & \No \\
    \option{yscale}            & Set \aTableau $y$-scale                             & \Yes & \Yes \\
    \bottomrule
  \end{xltabular}
  ...
  The \option{path box} sets the default entry that us used in box in
  each path. The \option{path box} is only used if you have not
  specified a box entry as a subscript in the path/ribbon specification.
  This key is useful if you want to mark all of the boxes in the path.
  ...
  In this way you have more control because different paths can be given
  different styling.
  ...
  Finally, \TikZ-styling can be added to ribbons using
  \option{ribbon style} and \option{ribbon box style}. The \option{ribbon style} changes the
  ...
  Use \keyword{\tikzset} whenever you are defining styles that will be
  used more than once.
  ...
  \option[]{dotted cols}[comma separated list of columns]
  \option*[]{dotted rows}[comma separated list of rows]

  The \option{dotted rows} and \option{dotted cols} keys can be used
  ...
  specified.  Instead of \option{dotted cols}, you can use the
  ...
  \option{snobs} do not affect the border of the diagram or tableau, and
  it is your responsibility to ensure that the boxes in the ribbon are
  inside the diagram or tableau.  This said, it is a useful feature to
  be able to place boxes outside of the diagrams.
  ...
  Here is a nice use of \option{paths} to add arrows to a ribbon
  tableau:
  ...
  \textsf{L} and \textsf{R}, which draw arrows to the left and right,
  respectively, where left and right is with respect to the two boxes in
  each path.  The two \keyword{\foreach} statements might look
  complicated, but they are just loops which add the entries in the
  tableau. These loops set \keyword{\r}, \keyword{\c} and \keyword{\l}
  equal to the row index, column index and label, respectively, using
  the following sequences of triples.

  \option[draw=\aTableauColour{Inner}, thin]{ribbon style}{\TikZ-styling}
  \option*[-]{ribbon box style}{\TikZ-styling}
  ...
  \option[]{ribbon box}{default box contents}
  ...
  instead of the current value of \option{ribbon box}. If a ribbon
  tableau contains multiple ribbons, then \option{ribbon box} is used
  for all ribbons.

  Similarly, these are keys
  \option{path style}, \option{path box}, \option{path box style},
  \option{snob style}, \option{snob box} and \option{snob box style} tha
  work in the same way. In addition, most keys for \keyword{\Tableau}
  work as expected for ribbon tableaux.
...
\end{example}
  ...
  \option[0]{xoffsets}[$|$-separated list of $x$-offsets]
  \option*[0]{yoffsets}[$|$-separated list of $y$-offsets]
  ...
  more complicated bead styles can be defined using \keyword{\tikzset}.
  ...
  You can remove beads by adding the \emph{no shade} style%
  \footnote{Surprisingly, \TikZ \emph{does not} provide
  a way to disable a shading that is already in effect. The
  \emph{no shade} style is based on a stack exchange post of
  \href{https://tex.stackexchange.com/a/85750/234252}{Jake}'s and it
  will work whenever the \aTableau package is loaded.}. In such cases,
  you probably want to replace the bead with a label, so you will also
  need to change the text, which is white by default.
  ...
  do not discuss all of these options here. In particular,
  \keyword{\Abacus} accepts the following keys:
  \option{align},
  \option{math entries},
  \option{text entries},
  \option{name},
  ...
  \option*[very thick, draw=\aTableauColour{Inner}]{abacus ends style}[\TikZ-styling]

  The \option{abacus ends} key sets the style used for the top and
  bottom of the abacus. The meaning of these five
  ...
  \option{abacus ends}. There are six options for the top of the
  abacus, and six options for the bottom, so there are 36 possible
  configurations. We will not list all of them, but here are some
  examples.
 ...
 As one more example, \emph{beta numbers} might be displayed using:

  \begin{example}
  \Abacus[abacus ends=**, east, rows=9]{1}{2,1^2,*0^3}
  \end{example}
  ...
  styling for the abacus runners, which is set using \option{runner
  style}. Changes to \option{runner style} also changes the style of the
  abacus ends, but their style is ultimately determined by
  \option{abacus ends style}.
  ...
  \option[ball color=\aTableauColour{StarStyle}, text=\aTableauColour{Main}]{abacus star style}
     [\TikZ-style keys]
  ...
  \option[\aTableauColour{Main}]{bead}[\LaTeX colour]
  ...
  \option[\textbackslash small]{bead font}[\LaTeX\ font command]
  ...
  \option[0.4]{bead size}[decimal number, distance in \unit{cm}]
  \option*[0.42]{bead sep}[decimal number, distance in \unit{cm}]
  \option*[0.42]{runner sep}[decimal number, distance in \unit{cm}]

  These three keys control the size of the beads and the distance
  between them, so they would normally be used togther. By default, the
  ...
  \option[circle]{bead style}[\TikZ styling]

  The keys above are usually sufficient for finetuning the style of the
  abacus beads. The \option{abacus style} key appends \TikZ-styling to
  the default bead style.

  \begin{example}
  \Abacus[bead style={shading=axis}, bead text=red] {3}{3_3,2^2,1^3}
  \end{example}

  \option[white]{bead text}[any colour]
  ...
  numbers for the bead specifications.

  \option[]{dotted cols}[list of runner indices]
  \option*[]{dotted rows}[list of row indices]

  \option[]{entries}[beta,residue,rows,shape]
  ...
  \begin{itemize}
    ...
    \item[beta]\index{entries!beta}
    Label the beads by their beta numbers
  \begin{example}
  \Abacus[entries=beta]{3}{3,2^2,0^2} \qquad
  \Abacus[entries=beta]{4}{3,2^2,0^2}
  \end{example}

  \item[residues]\index{entries!residues}
  Label the beads by their residues. Unlike for tableaux, it is not not
  necessary to specify the quantum characteristic \option{e}, because
  this is determined by the number of runners and the \option{cartan}
  type. (You can set \option{e} manually as well.)
  \begin{example}
  \Abacus[entries=residues,]{3}{3,2^2,0^2} \qquad
  \Abacus[entries=residues,cartan=C]{4}{3,2^2,0^2}
  \end{example}

  \item[rows]\label{entries!rows}
  Label the beads by their rows index of the corresponding part in the
  partition.
  \begin{example}
  \Abacus[entries=beta]{3}{3,2^2,0^2} \qquad
  \Abacus[entries=beta]{4}{3,2^2,0^2}
  ...
  \item[shape]\index{entries!partition}
  Label the beads by their shape, which is the corresponding partition.
  \begin{example}
  \Abacus[entries=shape]{3}{3,2^2,0^2} \qquad
  \Abacus[entries=shape]{4}{3,2^2,0^2}
  \end{example}
  ...
  disable this for individual beads using \keyword{no shade}. The
  \option{no shade} disables the \TikZ-ball shading for all beads,
  making it possible to fully customise how the abacus beads are
  displayed.

  \begin{example}
    \Abacus[no shade]{3}{3,2^2,0^2} \qquad
    \Abacus[bead style={no shade, shape=diamond, fill=brown, text=white}]{3}{3,2^2,0^2}
  \end{example}

  As the first example shows, if you use the \option{no shade} key then
  you need to define a style for the abacus beads. As the second example
  shows, rather than using \option{no shape} as an \aTableau-key you can
  use it directly as a \TikZ-style inside \option{bead style}. (We
  include the \option{no shade} partly for convenience and partly to
  advertise how it can be used.)

  \option[draw=\aTableauColour{Inner}]{runner}[\LaTeX colour]
  \option*[very thick]{runner style}[\TikZ-styling]
  ...
  \begin{example}
    ...
    \Abacus[runner=brown, bead=blue]{3}{6,4,3,[ball color=red]0^2}
  \end{example}

  Append \TikZ-styling keys to the style for the abacus runners.

  \begin{example}
    \Abacus[runner style={dashed}]{3}{3,2^2} \quad
  \end{example}

  \option[\aTableauColour{Inner}]{tick}[\LaTeX colour]
  \option*[semithick]{tick style}[\TikZ style]
  \option*[0.1]{tick length}[a decimal number]

  These three keys control the ticks on the abacus runners, which mark
  the empty bead positions.
  ...
  The entries of a tableau a usually single characters, but they can be
  more complicated. The paper
  ...
  \begin{example}
  \newcommand\Symb[2]{\begin{array}{l}#1\\#2\end{array}}
  ...
  the up and down strings to this diagram, taking as input the string
  ...
  non-trivial example showing that shows how to use the named nodes for
