...
% Modified: 23:34 Friday, 03 October 2025
% ----------------------------------------------------------------------
...
\begin{document}
 ...
 Some \Tableau[align=bottom]{124,357,68} words
  ...
  boxes in ribbon are drawn.  As the orange and yellow ribbons in
  example show, the style of an individual box in the ribbon takes precedence over
  ...
  Topologically, boundary of a generalised ribbon is a union of (oriented) circles.
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
 ...
 horizontal, runner using:
  ...
  \option[ball color=\aTableauColour{StarStyle}, text=\aTableauColour{Main}]{abacus star style=style}
     [\TikZ-styling]
  \index{star style!abacus star style}
  ...
  \aTableau commands. These commands are used in exactly the same ways
  ...
  \begin{xltabular}{0.9\textwidth}{l>{\raggedright}Xcc}\toprule
    ...
    \option{abacus star stle}  & Set the abacus star style                           & \No  & \Yes \\
  ...
  The \arXiv now uses \TeXLive2025 by default, so papers using \aTableau
  should compile on the \arXiv. Unfortunately, the \arXiv only has
  version 2.1.0 of \aTableau installed, as this is what shipped with
  \TeXLive2025. If \aTableau cause difficulties when compiling your file
  on the \arXiv, then you may need to upload the latest \aTableau style
  file, \texttt{atableau.sty}, with your submission.
...
we need to draw a tableau, connecting with with other combinatorial data.
