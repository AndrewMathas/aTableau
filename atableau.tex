...
% Modified: 16:17 Tuesday, 31 December 2024
% ----------------------------------------------------------------------
...
%   \Diagram{2,1^2}\qquad
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  The commands are designed to be easy to use, easy read, and be highly
  ...
  the \keyword{\Abacus} command), and tableaux are entered as comma-separated words. The commands support the main conventions for drawing tableaux and diagrams .
  By way of example, the following table shows how to draw a Young diagram of
  shape $\lambda=(4,3^2,2)$ using the four tableaux conventions
  supported by \aTableau:
  \begin{center}
      \aTabset{baseline=centre}
  ...
  some of these options together with the fact that every key
  ...
  being drawn. All of the options are described in detail in the following
  sections. The following options are common to all of the commands.

  \begin{options}
    \item[baseline]
    Sets the baseline for vertically aligning diagrams. The possible options
    are \keyword{baseline=top}, \keyword{baseline=center}, and
    \keyword{baseline=bottom}, which aligns the baseline of the diagram
  ...
  inside a \keyword{tikzpicture} environment by supplying Cartesian
  coordinates.
  ...
  The diagram is drawn twice, the first time without labels, to
  emphasize that most of the right-hand diagram is drawn by
  the~\keyword{\Diagram} command, even though most of the commands in
  the example are \TikZ commands. In particular, the shading of the
  boxes in the \emph{$(1,2)$-hook} and \emph{$(1,2)$-rim hook} is done
  by the \option{ribbons} key. In fact, this diagram could be drawn
  without using a \keyword{tikzpicture} environment by using the
  \option{tikz after} key to add the \keyword{\draw} commands.

  The top of the last example defines two \TikZ-styles, \textsf{B} and
  \text{G}, which add the \textsf{b}lue and \textsf{g}reen colour
  hatchings of the two hooks, respectively. Inside the
  \keyword{tikzpicture} environment, the Young diagram is placed at
  position $(0,0)$ by the \keyword{\Diagram} command. The three
  \keyword{\draw} commands are used to add the three labelled arrows to
  the picture. Please consult the very readable \TikZ manual for more
  details on the \TikZ-commands in this example.
  ...
  environment because it is equipped with the Cartesian coordinate
  $(0,0)$. Secondly, this example uses four \emph{named coordinates}:
  (\textsf{B-1-1}), (\textsf{B-1-2}), (\textsf{B-4-1}), and
  (\textsf{B-4-2}). When \aTableau draws a tableau, or a diagram, it
  creates a \TikZ named coordinate for every box in the diagram,
  where (\textsf{B-r-c}) is the box in row \textsf{r} and
  ...
  You can change the \textsf{B} in the name of these anchors to anything
  you like using the \option{name} option. Similar anchors are created
  for abacus beads.  These \textit{anchors} are a standard part of
  \TikZ.  If you change the shape of the node, then the available
  anchors may change; for more information see the \TikZ manual.
...
%  described in the following sections.
  ...
  instead of the default node names (\textsf{B-r-c}), which are used for
  the second diagram.
  ...
  This example shows that modifying the borders of a box does not really
  emphasis the box, so use this sparingly. Modifying borders is also
  problematic because boxes placed later are likely to overwrite the border (see
  \autoref{SS:order}). In addition, the orange circle in this example
  looks too big, but it is what we asked for because the diameter
  of the circle is the horizontal width of the box. The next example shows that
  we get a better result for \textit{Australian} and \textit{Ukrainian}
  tableaux if we add a \textit{minimum size} specification.
  ...
  \option[centre]{baseline}[centre/bottom/top]

  By default, the baseline of a tableau is its centre. This can be
  changed using the \option{baseline}.

  \begin{example}
    Some \Tableau[baseline=centre]{124,357,68} words % the default
  \end{example}

  \begin{example}
    Some \Tableau[baseline=top]{124,357,68} words
  \end{example}

  \begin{example}
   Some \Tableau[baseline=bottom]{124,357,68} words
  \end{example}

  Similarly, use \option{baseline} to control how tableaux are aligned
  in displayed equations.

  \begin{Example}
    \aTabset{baseline=bottom}
    \Tableau{124,357,68}\qquad
    \Tableau{124,35}
  \end{Example}

  \begin{Example}
    % by default: baseline=centre
    \Tableau{124,357,68}\qquad
    \Tableau{124,35}
  \end{Example}

  \begin{Example}
    \aTabset{baseline=top}
    \Tableau{124,357,68}\qquad
    \Tableau{124,35}
  \end{Example}
  ...
  \option[centre]{halign}[centre,left,right]
  \option*[centre]{valign}[bottom,centre,top]
  ...
  Use \option{inner colour=none}, to remove the inner tableau walls.

  \begin{example}
  \Tableau[ukrainian, inner colour=none]{123,45,67}
  \end{example}{}

  \option[]{label}[any character/text]
  \option*[font=\textbackslash scriptsize,text=\aTableauColour{Inner}]{label style}[\TikZ commands]
  ...
  Use the \option{math boxes} and \option{text boxes} to have the
  ...
  of the anchor names, as well as \autoref{Ex:ABnames} and
  \autoref{Ex:pointing}.
  ...
  \Tableau[star style={text=magenta,
     draw=cyan, thick}]{1*2*3,4*5} \quad
  ...
  shape.  In addition, many options assume that tableaux and diagrams do
  ...
  \begin{itemize}
    ...
    \item The \option{dotted rows} and \option{dotted columns} keys are
  ...
  This approach works, but it is cumbersome and it is easy to miscount,
  so \aTableau provides the \keyword{\Diagram} command.  The
  ...
  The \option{no border} option is provided only for completeness
  because it is the inverse of \option{border}. That is, \option{no
  border} is equivalent to \option{border=false}, and \option{no
  border=false} is equivalent to \option{border=true}.
  ...
  When \option{border only} is set to true the internal walls around boxes are not drawn.

  \begin{example}
  \Diagram[border only]{3,2,1} \qquad
  \Tableau[french, border only, no border]{123,45}
  \end{example}

  The \option{border only} key is ignored by the \keyword{\Tableau}
  command.
  ...
  \option[]{dotted columns}[comma separated list of columns]
  \option*[]{dotted rows}[comma separated list of rows]

  The \option{dotted rows} and \option{dotted columns} keys can be used
  to draw  diagrams that where the specified rows and columns are
  replaced with dots. This makes it possible to draw \emph{generic}
  diagrams, where the number of rows and columns are not fully
  specified.  Instead of \option{dotted columns}, you can use the
  short-hand key \option{dotted cols}.

  \begin{example}
  \aTabset{scale=0.8}
  \Diagram[dotted rows={2,4,5}]{4^2,2^2,1^3} \qquad
  \Diagram[dotted columns={5,3,4},
    border only] {6,5,4,3,2,1}
  \end{example}

  As shown above, when row and columns indices are in increasing
  ...
  \begin{example}
  \aTabset{scale=0.8, border only}
  \Diagram[dotted rows={4,5,3}] {6,5,5,5,2,1}
  \Diagram[dotted cols={5,3,4}] {6,5,5,5,2,1}
  \end{example}

  \index{tableau~dotted rows}
  \index{tableau~dotted columns}
  The \option{dotted rows} and \option{dotted columns} keys work by
  overwriting all of the content on the specified rows and columns, so
  anything that was drawn in these rows and columns will disappear.  The
  \option{dotted rows} and \option{dotted columns} keys can be used with
  ...
  When the \option{dotted rows} and \option{dotted columns} keys are
  used together, the \option{dotted rows} are applied first.

  \begin{example}
  \Diagram[scale=0.8, border only,
    dotted rows={4,5},
    dotted cols={4,5},
  ] {6,5,5,5,2,1}
  \end{example}
  ...
  choosing which rows and columns are removed because, otherwise,
  artifacts can remain. Ideally, the endpoints of the rows and columns
  being removed should not overlap. For example, the circled horizontal
  dots below are a ``user error'', not a bug, because these dots come
  from trying to use \option{dotted rows} to remove row three:
  ...
  The \option{dotted rows} and \option{dotted columns} keys are not
  ...
  \begin{itemize}
    ...
    using \option{type=C}, \option{type=AA}, and \option{type=DD},
    respectively.

    \begin{example}
    % affine type C
    \Diagram[ukrainian,show=residues, e=2, type=C]
      {8,4,2}
    \end{example}

    \begin{example}
    % twisted affine type A
    \Diagram[australian, show=residues, e=2, type=AA]
      {8,4,2}
    \end{example}

    \begin{example}
    % twisted affine type D
    \Diagram[french, show=residues, e=2, type=DD]
      {8,4,2}
    \end{example}
...
\]
  ...
  \Diagram[scale=0.3,border=false,baseline=bottom]{2^2}-square, and if
  ...
  \begin{example}
  \aTabset{baseline=top, no border}
...
\end{example}
  ...
  \option{right delimiter} to empty.
  ...
  \option[]{label}[a |-separated list of labels]
  ...
  way to represent partition. The abacus used particularly in modular
  representation theory, where the runner of runners is (quantum)
  characteristic.
  ...
  \begin{example}
     \Abacus{2}{4,1,1} \qquad
     \Abacus{3}{4,1,1} \qquad
     \Abacus{4}{4,1,1}
  \end{example}
  ...
  % --------------------------------------------------------------
  \subsection{Colours}\label{S:Colours}
