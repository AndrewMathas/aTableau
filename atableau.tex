...
% Modified: 14:22 Monday, 16 June 2025
% ----------------------------------------------------------------------
...
\begin{document}
  ...
  drawing the diagrams that I need in my own research. It is written to
  be easy to use and as
  flexible as possible in the hope that the package will draw the
  pictures\footnote{In this manual, ``diagram'' usually means ``Young
  diagram'', so a ``picture'' is anything drawn by an \aTableau command.}
  that others need.
  ...
  As deteailed below, it is possible to apply \TikZ-styling to the different
  ...
  \begin{options}
    ...
    environment. This is is equivalent to
  ...
  The left-hand picture shows that most of the right-hand picture is drawn
  ...
  \begin{warning}
     ...
     environment.  If you do accidentally add a semicolon, \TikZ\ will
  ...
  environment because it was given the Cartesian coordinate $(0,0)$. A
  ...
  necessary because the ribbons are drawn \textit{after} the tableau
  ...
  \footnote{As shown below, as better approach to define a
  ...
  only. I recommend against such garish choices in any self-respecting
  document!
  ...
  drawing of the drawing of the (thick) border wall around a tableau.
  ...
  \option{e} \textit{after} specifying the \option{colours}. For more
  information see the description of \option{entries=residues}, and the
  next example.
  ...
  The \option{colours} key has precedence over the \option{box fill}
  key, so these two keys should not be used in the same picture.
  ...
  example, the dashes on bottom of one box do not coincide with
  ...
  Use \keyword{\tikzset} whenever you are defining styles that are
  likely to be used more than once. Alternatively, you can use
  \keyword{\aTabset{styles={...}}}.
  ...
  \smallskip\noindent As shown by the last last tableau, the default
  minus sign for a negative number is too long when it appears inside a
  tableau.  To correct for this, \aTableau uses \keyword{\shortminus}
  for negative content values. In the second diagram above, \aTableau
  ...
  \option{shape} key to set the border of the ribbon tableau.
...
\end{example}
  ...
  \end{example}
    ...
    \item\option{entries=last}: Prints the \emph{last} \textbf{standard} tableau of this
    shape, which has the numbers $1,2\dots,n$ entered in order from top
  ...
  entries, afer which \option{snobs=...} is used to rewrite the
...
\end{Example}
  ...
  commands and their options. The earlier sections describe these commands and
  show how their options, or keys, are used with many examples.
  ...
  can default \aTableau values for the \aTableau options (and the
  \keyword{\shortminus} command for shorter minus signs, which are used
  for contents in tableaux).

  For easier reference, here is the complete list of \aTableau keys,
