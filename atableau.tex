...
% Modified: 00:21 Thursday, 19 December 2024
% ----------------------------------------------------------------------
...
%  described in the following sections.
  ...
  diagrams, where the number of rows and columns are not fully specified. If
  two, or more, of the row and columns are in increasing consecutive order, then the
  corresponding rows or columns are replaced as a block.

  \begin{example}
  \Diagram[dotted rows={2,4,5}]{4^2,2^2,1^3} \qquad
  \Diagram[scale=0.8, dotted rows={3,4,5},
    inner colour=none] {6,5,4,3,2,1}
  \end{example}

  \begin{example}
  \Diagram[scale=0.8, dotted rows={4,5,3},
    inner colour=none] {6,5,4,3,2,1}
  \Diagram[scale=0.8, dotted cols={3,4,5},
    inner colour=none] {6,5,4,3,2,1}
  \end{example}

  \index{tableau~dotted rows}
  \index{tableau~dotted columns}
  These keys work by overwriting all of the content on the specified rows and
  columns, so anything that was drawn in these will rows will disappear.
  The \option{dotted rows} and \option{dotted columns} keys can also be
  used with the \keyword{\Tableau} command, with the result that all of
  the boxes on the specified rows and columns will be removed.  Instead
  of \option{dotted columns}, you can use the short-hand \option{dotted
  cols}.
