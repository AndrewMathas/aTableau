...
% Modified: 16:21 Tuesday, 07 October 2025
% ----------------------------------------------------------------------
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
 ...
 horizontal, runner using:
  ...
  \option[-]{beads}[non-negative integer]

  Set the number of beads in the abacus. Normally, the number of beads
  on the abacus is automatically set to be the number of parts, or beta
  numbers, given to the \keyword{\Abacus} command. You can increase the
  number of beads by adding $0$'s to the partition, or, increasing the
  beta numbers. Alternatively, you can set the number of beads using
  \option{beads}. When the partition is entered as a quotient,
  \option{beads} set the number of beads on each runner.
  ...
  The \option{beads} is incompatible with \option{beta numbers}.
  ...
  \begin{xltabular}{0.9\textwidth}{l>{\raggedright}Xcc}\toprule
    ...
    \option{abacus star style} & Set the abacus star style                           & \No  & \Yes \\
  ...
  \TeXLive2025. If your document does not compile on the \arXiv because
  of \aTableau, you may need to upload the latest \aTableau style file,
  ...
  using the \aTableau package, with the aim of demonstrating how to use
  the package to construct some non-trivial, real-world  examples.
