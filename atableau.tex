...
% Modified: 21:19 Friday, 03 January 2025
% ----------------------------------------------------------------------
...
% ----------------------------------------------------------------------
\usepackage{bbding}
\newcommand\Yes{\textcolor{ForestGreen}{\CheckmarkBold}}
...
\NewDocumentCommand\AddOptionLink{m}
{
   \tl_set:Nx \l_tmpc_tl {Option:\str_lowercase:f{#1}}
   \prop_if_in:NVF \l__atableau_options_prop \l_tmpc_tl
   {
      \phantomsection\label{Option:\str_lowercase:n{#1}}% add a label for references
      \prop_put:NVn \l__atableau_options_prop \l_tmpc_tl {}
   }
}
% \option*[default value]{option name}[accepts]
% The *-version does not do a \bigskip before the option. A hyperlink is
% added for new displayed options
...
{%
  ...
  {%
    ...
    \tl_set:Nn \l_tmpb_tl {Option:\str_lowercase:f{\l_tmpa_tl}}
    \hyperref[\l_tmpb_tl]{\textsf{\color{aTableauInner}\l_tmpa_tl}}%
    \index{\l_tmpa_tl}%
  ...
  {%
    \IfBooleanTF{#1}{\newline}{\bigskip}%
    \AddOptionLink{#3}% add a label for references
...
%   \Diagram{2,1^2}\qquad
  ...
  \keyword{tikzpicture} environment.  This package grew out of my own
  research, so it does all of the things that I want it to do. On the
  other hand, it is written as flexibly as possible in the hope that it
  will draw the diagrams that others need.

  For the impatient, here is a basic example that shows how to use the main
  commands provided by the \aTableau package:
  ...
  the \keyword{\aTabset} command. Options can also be applied to
  individual commands.
  \indexcmd{aTabset}

  Each of the \aTableau commands is designed to be easy to use and easy to read,
  and it is highly customisable. Each command accepts an optional list
  of key-value options that modify the diagrams. In addition, optional
  ...
  \keyword{tikzpicture} environment, making makes it possible to use
  these commands as part of more complicated diagrams.
  ...
  The $(x,y)$-Cartesian coordinates are optional, being necessary if
  (and only if), the diagram is part of a~\keyword{tikzpicture}
  environment.  Similarly, the key-value \textit{options} control the
  style and appearance of the final diagram.
  ...
  being drawn. The full list of options is given in \autoref{S:keys}.
  The following options are common to all \aTableau commands.
  ...
  \option{border} is the same as \textsf{border=true} (and \option{no
  border} is the same as \textsf{border=false}).  In all of the options,
  American spelling variations, such as \emph{center} (instead of
  \emph{centre}), \emph{color} (instead of \emph{colour}) et cetera, are
  tolerated. All of the colours in \aTableau diagrams can be changed
  using the key-value options. The names of the underlying base colours
  used by \aTableau are given in \autoref{S:Colours}.
  ...
  The diagram is drawn twice, both outside and inside a
  \keyword{tikzpicture} environment. The first diagram, which does not
  have labels, emphasizes that most of the right-hand diagram is drawn by
  ...
  without using a \keyword{tikzpicture} environment by adding
   the \keyword{\draw}-commands to the \option{tikz after} key.)
  ...
  $(0,0)$. The second important point is that this example uses four
  \emph{named coordinates}: (\textsf{B-1-1}), (\textsf{B-1-2}),
  (\textsf{B-4-1}), and (\textsf{B-4-2}). When \aTableau draws a
  tableau, or a diagram, it creates a \TikZ named coordinate
  (\textsf{B-r-c}) for every box in the diagram, where (\textsf{B-r-c})
  is the box in row~\textsf{r} and column~\textsf{c}. These named
  ...
  You can change the \textsf{B} in the name of these anchors using the
...
%  described in the following sections.
  ...
  Notice that even though the \keyword{\SkewTableau} commands are being used inside a
  ...
  \option{text boxes} key, which is described below.
  ...
  tableau entries.  To take advantage of these styles you need to have
  at least rudimentary knowledge of \TikZ-styling. The examples in this
  manual give a good indication of what is possible.  For anything more
  exotic, please consult the \TikZ manual.
  ...
  \keyword{fill=}\aTableauColour{StarStyle}. You can change the default
  $*$-style using the \option{star style} key:
  ...
  in a tableau, but each of these entries will be given the same style.
  ...
  because boxes placed later are likely to overwrite the border (see
  \autoref{SS:order}). As a result, modifying the borders of the boxes
  is not a good way to add emphasis to a tableau,  so use this
  sparingly. In addition, the orange circle in this example looks too
  big, but it is what we asked for because the diameter of the circle is
  ...
  changed using the \option{align}.

  \begin{example}
    Some \Tableau[align=centre]{124,357,68} words % the default
  \end{example}
  ...
  the drawing of the (thick) border wall around a tableau. As with other boolean
  ...
  \autoref{Ex:pointing} for examples using node names and anchors.

  \option[]{ribbons}[list of ribbon specifications]
  \option*[]{snobs}[list of ribbon specifications]
  \option*[draw=\aTableauColour{Inner},thin]{ribbon style}[\TikZ-styling commands]
  ...
  \textit{ribbons} backwards gives \textit{snob}bir.) The \option{ribbon
  style} key controls the style of the ribbons.
  ...
  Use the \option{ribbon box} option to change the default style of the
  ...
  Finally, the \option{ribbon style} changes the default style of every
  ribbon.
  ...
  the internal boxes are drawn.  When \option{no boxes} is in force, the
  internal walls around boxes are not drawn.
  ...
  to draw  diagrams where the specified rows and columns are
  replaced with dots. This makes it possible to draw \emph{generic}
  diagrams, where the number of rows and columns is not fully
  specified.  Instead of \option{dotted columns}, you can use the
  equivalent short-hand key \option{dotted cols}.
  ...
  all boxes, with their contents, ghat are in the specified rows and columns.
  ...
  with the \option{snobs} being added at the very end.
  ...
  The \option{entries} option provides a shortcut for drawing some common
  tableaux of a specified shape.

  \option[]{entries}[contents,first,hooks,last,residues]
  ...
  \item\option{entries=contents} where each box is
  ...
  \noindent\AddOptionLink{cartan}\AddOptionLink{e}
  ...
  \Diagram[australian, entries=residues, e=2, cartan=AA]
    {8,4,2}
  \end{example}
  ...
  \option[draw=\aTableauColour{Skew}, fill=\aTableauColour{SkewFill}, thick]{skew border style}[\TikZ-styling]
  ...
  \option*[thin, fill=\aTableauColour{SkewFill}]{skew box style}[\TikZ-styling]
  ...
  The definition of hook lengths for shifted diagrams can be found in
  the literature; for example, see \cite{Konvalinka:ShiftedHookLengths}!
  ...
  ensure that the boxes in the ribbon not have negative column indices.
  Inside a \keyword{\Diagram} or \keyword{\Tableau} command, any ribbons
  added using \option{ribbons}, or \option{snobs}, do not affect the
  border of the diagram or tableau, and it is your responsibility to
  ensure that the boxes in the ribbon are inside the diagram or tableau.
  This said, it is a useful feature to be able to place boxes outside of
  the diagrams, which can be done using \option{ribbons} for
  \keyword{\Tableau} or \option{snobs} as these do not contribute to the
  ...
  ribbons, respectively. By default, ribbons have an inner wall and the
...
\end{example}
  ...
  \option[]{entries}[contents,first,hooks,last,residues]
  \option*[]{charge}[$|$-separated list of charges]
  \index{contents}
  \index{residues}
  ...
  Use the \option{entries} key to draw particular types of tableau of the
  specified shape. Use \option{charge} to provide offsets in each
  component when using \option{entries=contents} and
  \option{entries=residues} (and no charge should be set when using
  \option{entries=first} and \option{entries=last}).
  ...
  \end{example}
    ...
    shape, which has the numbers $1,2\dots,n$ entered in order from top
    to bottom down successive columns, and right to right left along components (with appropriate modifications for
    the non-English conventions).
  ...
  can be specified using \option{cartan=??}.
  ...
  No extra space is allowed for the labels, so use \option{xoffsets}
  and \option{yoffsets} to adjust for wide labels.
  ...
  changed like using the \option{name} key.
  ...
  however, such diagrams can be be drawn using an appropriate
  ...
  \begin{Example}
  \aTabset{align=top}
  \Abacus{1}{2,1,1}     \quad \Abacus{2}{2,1^2,0^3} \quad \Abacus{3}{2,1^2,0^5} \quad
  \Abacus{4}{2,1^2,0^5} \quad \Abacus{5}{2,1^2,0^5} \quad \Abacus{6}{2,1^2,0^5} \quad
  \Abacus{7}{2,1^2,0^5} \quad \Abacus{8}{2,1^2,0^5}
  \end{Example}
  ...
  In particular, the two beads corresponding to \keyword{1^2} are
  coloured red, whereas the other three beads that correspond to a~$1$
  are orange and the off-white colour, which is the colour given by the
  \option{abacus star style}. As with the tableaux commands, more
  complicated bead styles can be defined using \keyword{\tikzset}.

  In addition, to applying style to the individual beads on the abacus,
  labels for the beads can (optionally) be given as subscripts in the
  ...
  \Abacus{3}{5_5, 4_4, 3_3, 2_2,0_0} \quad
  \Abacus{4}{*5^2_5, [R]4^3_4, 3, 2_2,*0^2_{oo}} \quad
  \Abacus{5}{8^2_-,7_+,5_a,4^2_b,1_c^3,1_d}
  \end{example}

  The bead labels are arbitrary, however, anything winder than the bead
  will spill into the abacus. Traditionally, the beads are unlabelled,
  but your can give optional labels to as many beads as you like.

  % --------------------------------------------------------------
  \subsection{Abacus keys}

  The abacus keys, or options, control the general appearance of the
  abacuses drawn by the \keyword{\Abacus} command. Many of the options, or keys, for
  tableaux and diagrams can also be applied to abacuses, including
  \option{align},
  \option{name},
  \option{scale},
  \option{tikz after},
  \option{tikz before}, and
  \option{tikzpicture}. See \autoref{S:keys} for the complete list.

  \option[true]{south}{true/false}
  \option*[false]{north}{false/true}
  \option*[false]{east}{false/true}
  \option*[false]{west}{false/true}

  Abacuses, just like tableaux, have four different conventions.  By
  default, abacuses are drawn in the southerly direction, from top to
  bottom. Using these keys, abacuses can also be drawn to the north,
  east and west.

  \begin{example}
    \Abacus[south]{2}{4^3,2,1} \qquad % the default
    \Abacus[east]{2}{4^3,2,1}  \par\smallskip\par
    \Abacus[north]{2}{4^3,2,1} \qquad
    \Abacus[west]{2}{4^3,2,1}
  \end{example}


  \option[text=\aTableauColour{Main},text=\aTableauColour{StarStyle}]{abacus star style}
     [\TikZ-style keys]

  The \option{abacus star style} key appends \TikZ-style kleys to the
  $*$-style for beads.

  \begin{example}
  \Abacus{3}{5,4,*1^2,0^4} \qquad
  ...
  \option[\aTableauColour{Main}]{bead}[]
  ...
  abacus. As shown, use the \TikZ \keyword{ball colour} styling to
  change the colour of individual beads.

    \option[\scriptsize]{bead font}[]

    \option[0.3]{bead size}[bead diameter in \unit{cm}]

    \option[0.35]{bead sep}[]

    \option[white]{bead text}[]

    \option[false]{beta numbers}[false/true]

    When \option{beta numbers} is true, the numbers in the bead
    specifications should be entered as \emph{beta numbers}. The beta
    numbers give the bead positions, which are numbered $0,1,\dots$,
    starting from the first row and then continuing in this way in
    subsequent rows. Up to shift, bead numbers are first column hook
    lengths. In particular, beta numbers are pairwise distinct.

  \begin{example}
    \Abacus{3}{3,2^2} \quad
    \Abacus[beta numbers]{3}{5,3,2} \quad
    \Abacus[beta numbers]{3}{5,4,3,0}
  \end{example}

    \option[\aTableauColour{Inner}]{runner}[]

    \option[0.35]{runner sep}[]

    \option[\aTableauColour{Inner}]{tick}[]

    \option[0.1]{tick width}[]
  ...
  % --------------------------------------------------------------
  \section{List of \aTableau commands and options}\label{S:keys}
  \rowcolors{2}{aTableauSkewFill}{}
  ...
  In addition, the \keyword{\aTabset} command can be used to set default
  \aTableau options.

  For easier reference, here is the list of \aTableau keys, together
  ...
  abacuses --- or both! Although not listed, American spelling
  variations of all keys are tolerated.

  \begin{xltabular}{\textwidth}{lXcc}
    \toprule
    Key & Meaning & Tableau & Abacus \\
    \midrule
    \endhead
    \option{abacus star style} & Set \TikZ-style of abacus $*$-nodes               & \No  & \Yes\\
    \option{align}             & Set baseline alignment of \aTableau diagram       & \Yes & \Yes\\
    \option{australian}        & Use the Australian convention for tableau         & \Yes & \No \\
    \option{bead height}       & Set row height between beads                      & \No  & \Yes \\
    \option{bead width}        & Set column height between beads                   & \No  & \Yes \\
    \option{bead}              &                                                   & \No  & \Yes \\
    \option{bead font}         &                                                   & \No  & \Yes \\
    \option{bead sep}          &                                                   & \No  & \Yes \\
    \option{bead size}         &                                                   & \No  & \Yes \\
    \option{bead text}         &                                                   & \No  & \Yes \\
    \option{beta~numbers}      &                                                   & \No  & \Yes \\
    ...
    \option{box fill}          & Set tableau box fill colour                       & \Yes & \No \\
    \option{box font}          & Set tableau box font                              & \Yes & \No \\
    \option{box height}        & Set tableau box height                            & \Yes & \No \\
    \option{box text}          & Set tableau box text colour                       & \Yes & \No \\
    \option{box width}         & Set tableau box width                             & \Yes & \No \\
    \option{boxes}             & Draw inner walls around boxes                     & \Yes & \No \\
    \option{cartan}            & Set Cartan type for residues                      & \Yes & \No \\
    \option{charge}            & Set charge for contents and residues              & \Yes & \Yes \\
    ...
    \option{dotted cols}       & Specify diagram columns to replace with dots      & \Yes & \No \\
    \option{dotted columns}    & Specify diagram columns to replace with dots      & \Yes & \No \\
    \option{dotted rows}       & Specify diagram rows to replace with dots         & \Yes & \No \\
    \option{east}              & Abacus grows in the easterly direction            & \Yes & \No \\
    \option{empty}             & Symbol for empty tableau in multitableau          & \Yes & \No \\
    \option{english}           & Use the English convention for tableau            & \Yes & \No \\
    \option{entries}           & Draw contents/residues/hooks/first/last tableau   & \Yes & \No \\
    \option{e}                 & Set the quantum characteristic for residues       & \Yes & \Yes \\
    \option{french}            & Use the French convention for tableau             & \Yes & \No \\
    \option{halign}            & Horizontal alignment of box entries and bead labels & \Yes & \Yes \\
    \option{inner colour}      & Set colour of inner tableau wall                  & \Yes & \No \\
    ...
    \option{math boxes}        & Typeset box entries in math-mode (true by default)& \Yes & \No \\
    \option{name}              & Prefix of named nodes                             & \Yes & \Yes \\
    \option{no border}         & Disable tableau border wall                       & \Yes & \No \\
    \option{no boxes}          & Do not draw inner walls around boxes              & \Yes & \No \\
    \option{no separators}     & Disable separators in multitableau                & \Yes & \No \\
    \option{no skew border}    & Disable skew border                               & \Yes & \No \\
    \option{no skew boxes}     & Disable drawing of skew boxes                     & \Yes & \No \\
    \option{north}             & Abacus grows in the northerly direction           & \Yes & \No \\
    \option{ribbon box}        & Set style of boxes in ribbons                     & \Yes & \No \\
    \option{ribbon style}      & Set ribbon style                                  & \Yes & \No \\
    \option{ribbons}           & Comma separated list of ribbons to add to tableau & \Yes & \No \\
    \option{right delimiter}   & Set left delimiter for multitableau               & \Yes & \No \\
    \option{rows}              & Set number of rows in multitableau or abacus      & \Yes & \Yes \\
    \option{runner}            & Set the colour of the abacus runners              & \No  & \Yes \\
    \option{runner~sep}        & Set distance between abacus runners               & \No  & \Yes \\
    \option{scale}             & Sets the \aTableau scale                          & \Yes & \Yes \\
    \option{separation}        & Set distance between component tableau in multitableau & \Yes & \No \\
    \option{separator colour}  & Set separator colour in multitable                & \Yes & \No \\
    \option{separators}        & Enable separators in multitableau                 & \Yes & \No \\
    \option{separator}         & Set separator colour in multitableau              & \Yes & \No \\
    \option{shifted}           & Set true for a shifted tableau                    & \Yes & \No \\
    ...
    \option{snobs}             & Comma separated list of snobs to add to tableau   & \Yes & \No \\
    \option{south}             & Abacus grows in the southerly direction           & \Yes & \No \\
    \option{star style}        & Set \TikZ-style of tableau $*$-nodes              & \Yes & \No \\
    \option{tabloid}           & Set true for a shifted tableau                    & \Yes & \No \\
    \option{text boxes}        & labels typeset in text-mode (false by default)    & \Yes & \Yes \\
    \option{tick}              & Set the colour of the abacus runner ticks         & \No  & \Yes \\
    \option{tick~width}        & Set the width of the abacus runners               & \No  & \Yes \\
    \option{tikz after}        & \TikZ-code injected after abacus                  & \Yes & \Yes \\
    \option{tikz before}       & \TikZ-code injected before abacus                 & \Yes & \Yes \\
    \option{tikzpicture}       & Add tikzpicture environment keys                  & \Yes & \No \\
    \option{ukrainian}         & Use the Ukrainian convention for tableau          & \Yes & \No \\
    \option{valign}            & Set vertical alignment of box entries and bead labels & \Yes & \Yes \\
    \option{west}              & Abacus grows in the westerly direction            & \Yes & \No \\
    \option{xoffsets}          & Set the $x$-offsets for component diagrams in a multitableau & \Yes & \No \\
    \option{xscale}            & Set the \aTableau $x$-scale                       & \Yes & \Yes \\
    \option{yoffsets}          & Set the $y$-offsets for component diagrams in a multitableau & \Yes & \No \\
    \option{yscale}            & Set the \aTableau $y$-scale                       & \Yes & \Yes \\
    \bottomrule
  \end{xltabular}
  ...
  in the literature. All of these diagrams predate this package. The
  aim of this section is not to showcase uses of this package but,
  rather, to show how some preexisting diagrams could have been drawn.

  Diagrams like the following one are common. Note that the $\alpha$ is
  added in row~$6$ and column~$8$ using a ribbon.
  ...
  These two ribbon tableaux can be found in Leclerc and Thibon's
  paper~\cite{LeclercThibon:}
  \begin{example}
  ...
  \end{example}
  The following tableau comes from Fayers
  paper~\cite{Fayers:DyckTilings}, which is uses Dyck tilings to
  understand homogeneous Garnir relations for KLR algenras of type~$A$.
  ...
  cannot replicate your problem, then it is unlikely that I will be
  able to fix it.
