...
% Modified: 15:43 Friday, 04 April 2025
% ----------------------------------------------------------------------
...
\begin{document}
  ...
  \begin{center}
    ...
    \begin{tikzpicture}
      \Tableau(0,0)[yscale=2]{[R]1[B]2,[G]3,[Y]4} \quad
      \Tableau(2.25,1.75)[australian]{[R]1[B]2,[G]3,[Y]4} \quad
      \Tableau(3.5,1.75)[skew={0,2}]{[Y]4[G]3[R]1,[B]2} \quad
      \Tableau(6,1.55)[ukrainian,skew={0,1^2}]{[B]2[R]1,[G]3,[Y]4} \quad
      \Tableau(8,-1)[yscale=2,french,skew={0,1^2}]{[B]2[R]1,[G]3,[Y]4} \quad
      \Tableau(6.0,-3.2)[ukrainian]{[R]1[G]3[Y]4,[B]2} \quad
      \Tableau(3.5,-3.2)[french]{[R]1[G]3[Y]4,[B]2} \quad
      \Tableau(2,-3.2)[australian]{[B]2,[R]1[G]3[Y]4} \quad
      \Abacus(2,0.5)[abacus ends=->,scale=1.4]{3}{[b]9,[w]8,[r]8,[b]6,[b]6,[r]2,[b]1}
      \Abacus(5,0.5)[traditional, framed,scale=1.4]{3}{[b]9,[w]8,[r]8,[b]6,[b]6,[r]2,[b]1}
    \end{tikzpicture}
  ...
  \textcolor{orange}{false}, the value can typically be omitted, and there is
  usually an \textit{inverse key}, which is given by adding a
  \textcolor{aTableauInner}{no}-prefix. For example, \option{border} is
  equivalent to \option{border=true} and to \option{no border=false}.
  The \option{no border} key is inverse to \option{border}, so
  \option{no border} is equivalent to \option{no border=true}, and to
  \option{border=false}.
  ...
  The left-hand diagram shows that most of the right-hand diagram is drawn
  ...
  \begin{warning}
     Unlike \TikZ-commands, you should \emph{not} put a semicolon
     after an \aTableau-command when it is used with Cartesian
     coordinates inside a \keyword{tikzpicture} environment. If you
     accidentally add a semicolon then, most of the time it will not
     cause any harm and \TikZ will just give you a warning.
  \end{warning}

  \TikZ-styling can be applied to the most of the components of the
  pictures produced by \aTableau commands.  The top of the last example
  defines two \TikZ-styles, \textsf{B} and \text{G}, which add the
  \textsf{b}lue and \textsf{g}reen coloured hatchings styling to the two
  ...
  \option{e}-residue into each box in these tableaux. The
  ...
  % --------------------------------------------------------------
    ...
    \begin{options}
      ...
      \item[*options] Optional arguments, or keys. The are described below, with examples.
  ...
  \option{text entries} key, as described below.
  ...
  \TikZ-styling \keyword{fill=}\aTableauColour{StarStyle} by default. You can
  ...
  The \option{label} keys should help match each tableau to the
  ...
  defining these conventions precisely, we use the tried and true method
...
\end{Example}
  ...
  The default colour theme for the pictures drawn by \aTableau are built
  from six colours:
    aTableauMain,
    aTableauInner,
    aTableauStarStyle,
  ...
  subjective. The \option{colour theme} key makes the colour equal to
  one of the following ``standard'' colour themes.
