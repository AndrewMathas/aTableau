% ----------------------------------------------------------------------
% aTableau manual - Andrew Mathas (C) 2022-2026
% Modified: 14:09 Tuesday, 30 June 2026
% ----------------------------------------------------------------------
...
% The property list \l__aTableau_meta_prop, and access function \aTableauMeta
% are defined in atableau.sty

% map the key-value pairs in the ini file to property entries in \l__aTableau_meta_prop
\ior_open:NnT \g_tmpa_ior { atableau.ini }
{   % ini file found
    \ior_str_map_inline:Nn \g_tmpa_ior
    {
        ...
        {   % Have: key = value
            ...
            % When the version takes the form 2.1.1-rc1 we need
            % to replace the dash with a dot, so we do this always
            \str_set:Ne \l_tmpa_str { \seq_item:Nn \l_tmpa_seq {2}}
            \str_replace_once:Nnn \l_tmpa_str {-} {.}
            \prop_put:NeV \l__aTableau_meta_prop { \seq_item:Nn \l_tmpa_seq {1} } \l_tmpa_str
        }
    }
...
% the key applies to tableaux or abacuses (or both), with \Yes adding a
...
% 2. When #2 (=default value) is omitted, this is an inline citation of
%    the key. A hyperlink is added
...
% environments that use \SaveExample to save the examples to a file
\newif\ifTesting

% Enable \ifTesting for atableau-testing.tex, which is a symbolic link
% to atableau.tex
...
\def\@oddfoot {\color{aTableauMain} Version \aTableauMeta{version}~(\aTableauMeta{release_date})\hfill\thepage}
\def\@evenfoot{\color{aTableauMain} \thepage\hfill Version \aTableauMeta{version}~(\aTableauMeta{release_date})}
...
\DeclareTotalTCBox\keyword{ O{} v }{
  ...
  verbatim,
  on line,
  top=-0.25pt,bottom=-0.25pt,left=0mm,right=0mm,
  colback=Ivory,
  colupper=aTableauMain,
  #1}
{\smash{\detokenize{#2}}}
...
% define the heading as a command because it contains \aTableauMeta{version}
\newcommand\aTableauHeader{%
  \begin{center}
    \begin{tikzpicture}
      ...
      \end{scope}
      \node[anchor=west,boxes] at (4cm,0cm) { \aTableauMeta{authors} };
      \node[anchor=east,boxes] at (\paperwidth-4cm,0) {Version \aTableauMeta{version}};
    \end{tikzpicture}
  \end{center}
}
...
\hypersetup{
  ...
  pdfinfo={
    Author  ={\aTableauMeta{authors}     },
    Keywords={\aTableauMeta{keywords}},
    License ={\aTableauMeta{license}     },
    Subject ={\aTableauMeta{description} },
    Title   ={aTableau manual - \aTableauMeta{version} },
    CreationDate={\aTableauMeta{release_date}},
    Copyright={\aTableauMeta{copyright} \aTableauMeta{authors}},
  },
  colorlinks=true,
  linkcolor=blue,
  citecolor=FireBrick,
...
\ExplSyntaxOn
% define the authors, adding a URL if one is specified in the ini file
\prop_if_in:NnTF \l__aTableau_meta_prop {author_url}
{ \newcommand\aTableauAuthor{\href{https\c_colon_str//\aTableauMeta{author_url}}{\aTableauMeta{authors}}} }
{ \newcommand\aTableauAuthor{\aTableauMeta{authors}} }
\ExplSyntaxOff
\makeatother
...
\begin{document}
  ...
  \begin{center}
    ...
    \aTabset{
      ...
      styles = {
        ...
        G = {fill=green!80!blue,circle,minimum size=3mm},
    ...
    \begin{minipage}{0.75\textwidth}
      \tableofcontents
      \footnotetext{\copyright\,Copyright \aTableauMeta{copyright}
        \quad\aTableauAuthor\\
        License: \href{https://\aTableauMeta{license_url}}{\aTableauMeta{license}}
      }
    \end{minipage}
  ...
  The \aTableau package started as code for drawing tableaux in my
  research. The package provides commands for drawing common objects in
  algebraic combinatorics and representation theory, together with
  styling options and a key-value interface for additional
  customisation. Internally, everything is drawn using \TikZ (and
  \LaTeX3). All \aTableau commands can be used as standalone
  commands, and as part of a \keyword{tikzpicture} environment. The
  commands are intended to be both easy to use and easy to customise, so
  that they can draw the wide variety of tableaux and abacuses found in
  the literature.\footnote{In this manual, a ``picture'' is anything
  drawn by an \aTableau command, whereas a ``diagram'' means ``Young
  diagram''.}

  For the impatient, here is an example that shows how to use the
  ...
  \cite{Fulton:Tableaux,Macdonald,Mathas:ULect} if you need them. For
  more advanced usage, such as adding styling, I recommend skimming
  through the manual and looking at the many examples.  \autoref{S:keys}
  gives a summary of the \aTableau commands and their options.
  ...
  decreasing list of positive integers, or using exponential
      ...
      \begin{longtable}{lp{0.6\textwidth}l} \toprule % longtable allows footnotes
        ...
        \textcolor{aTableauMain}{Convention} &\textcolor{aTableauMain}{\aTableau command} & \textcolor{aTableauMain}{Picture} \\ \midrule
         \showrowcolors
          ...
          Russian
          \footnote{These tableaux were introduced by Kerov and Ver\v{s}ik \cite{KerovVershik}.
          Earlier versions of this package called these tableaux \textit{Ukrainian tableaux},
          in protest over the Russian invasion of Ukraine. It is unreasonable to punish
          these authors for their government's aggression, so the package now uses the
          more common, and historically accurate, nomenclature from the literature. The
          \option{ukrainian}(russian) option
          is still supported.}
  ...
  order of their popularity in the literature.  By
  ...
  \end{example}
  As this example illustrates, partitions can either be typed as a
  ...
  Here, the \textit{options} are a comma-separated list of \aTableau
  options, or keys, that can be used to specify the default settings for
  your \aTableau pictures. The possible options are described below. As
  an example, to make the tableau and Young diagram commands use the
  ...
  key-value specifications, which modify and embellish the picture drawn
  by the command. In addition, optional styling can be applied to entries
  of a tableau, and the beads on an abacus.  Each command can be used as
  a standalone object in a document, or it can be incorporated into a
  ...
  \index{coordinates}
  \index{coordinates!Cartesian coordinates}
  \index{coordinates!polar coordinates}
  ...
  coordinates are always specified as Cartesian coordinates. In fact,
  arbitrary \TikZ coordinates can be used to position \aTableau pictures
  inside a \texttt{tikzpicture} environment, including polar coordinates
  and named coordinates. See \autoref{SS:coordinates} and
  \autoref{SS:abcoordinates} for more detail about using these
  coordinates for tableaux and abacuses, respectively.

  The \aTableau package provides eleven commands for drawing
  pictures:
  \begin{center}
  ...
  \begin{itemize}[nosep,topsep=-4mm, after=\medskip]
    ...
    by \option{entries=contents} when drawing content
    tableaux.\cprotect\footnote{The \aTableau package only defines the
  \keyword{\shortminus} command if it does not already exist. When it
  does not exist, the \keyword{\shortminus} command is defined using
  ...
  \begin{options}
    ...
    A shorthand for defining \TikZ-styles.
  ...
  With boolean options, which take values \textcolor{orange}{true} or
  ...
  The \aTableau keys tolerate American spelling variations. For example,
  in \aTableau keys you can use \emph{center} and \emph{color} instead
  of the correct spellings of \emph{centre} and \emph{colour}, respectively.
  \begin{warning}
  ...
  explicitly, the second \keyword{\Diagram} command uses coordinates to
  position the picture inside a \keyword{tikzpicture} environment.
  Otherwise, the two \keyword{\Diagram} commands in \autoref{Ex:diagramCoordinates} below
  are identical, except that the second one is embellished with labels
  that are drawn by the other $\TikZ$-commands inside the
  ...
  Cartesian coordinate $(0,0)$. Most of the right-hand picture is drawn
  ...
  and \textsf{G}, are defined. These styles are used to add the \textsf{b}lue and
  ...
  An important feature of \autoref{Ex:diagramCoordinates} is the use of four
  ...
  are available, depending on the tableau, or abacus, convention being used.
  ...
  The coordinate $(0,0)$ is needed in the last example because the
  \keyword{\Diagram} command is used in a \keyword{tikzpicture}
  environment. Rather than (0,0), any \TikZ coordinate could have been
  used. (As this example uses relative coordinates, a different choice of coordinate
  will not actually change the picture.) The use of coordinates becomes more
  meaningful when the \keyword{tikzpicture} environment has more
  components. In the next example, the key \option{name=B} makes the
  named coordinates for the second diagram take the form
  (\textsf{B-r-c}), instead of the default named coordinates
  (\textsf{A-r-c}), which are used in the first diagram. This makes it
  easy to draw a line between two boxes in these tableaux.
  ...
  inside a \TikZ \emph{matrix of nodes} to produce the following lattice of
  shifted tableaux:
  ...
  \begin{warning}
    ...
    because it relies heavily on \ctan{LaTeX3}. You will need to update
    your \LaTeX\ installation if it is older than \TeXLive 2024.
    Ideally, you should always use the latest release of your preferred
    \LaTeX\ distribution.
  \end{warning}
  ...
  integers. They are fundamental objects in algebraic combinatorics and
  representation theory.\index{partition} For example,
  $\lambda=(4,3,3,2,0,0,\dots)$ is a partition of $12$, because the
  entries sum to~$12$. When writing partitions, it is customary to use
  exponents for repeated parts, and to omit the trailing zeros ---~in
  fact, \aTableau requires you to omit the zeros. For example,
  we can write the partition $\lambda$ in \emph{exponential notation} as
  $(4,3^2,2)$.
  ...
  \]
  The (Young) diagram is usually visualised as an array of \emph{boxes},
  or \emph{nodes}, in the plane (see \autoref{S:Diagram}).  A
  \emph{tableau} is a diagram where the boxes are labelled using some
  alphabet. Equivalently, a diagram is a tableau with empty labels.
  ...
  than one convention in different parts of the manuscript.
  ...
  \begin{options}
    \item[*$(x,y)$]\index{xy@$(x,y)$} The  $(x,y)$-coordinates are
    needed only if the tableau is part of a \keyword{tikzpicture}
    environment. Arbitrary \TikZ coordinates can be used, including
    Cartesian coordinates (as shown), polar coordinates, and named nodes.
    \item[*options] Optional arguments, or keys, that are described below, with examples.
  ...
  In this tableau, there is an asterisk \textit{before} each of the entries $2$,
  $3$, $4$, $5$, $7$, and $11$, which gives the corresponding boxes the
  \option{tableau star} style. By default, boxes marked with a~$*$ are given the
  ...
  of the tableau entries in the last example appear to be blank.  If you are
  ...
  because \aTableau is smart enough to match square brackets when reading style.}.
  If you use a style more than once, then a better approach is to
  define a \TikZ-style for ``complicated'' styles because they are
  easier to read, proofread, and type.
  ...
  This example demonstrates that modifying the borders of a box is
  problematic because boxes placed later are likely to overwrite the new
  border style (see \autoref{SS:order}). As a result, modifying the borders
  of individual boxes is often not a good way to add emphasis to the boxes
  ...
  and easier to modify. If you need to define a style for a single
  picture, then you can also use the \aTableau \option{styles} key.
  ...
  tableau. (Arbitrary \TikZ coordinates can be used.) The following
  example shows how tableaux are placed using $(x,y)$-coordinates inside
  a \keyword{tikzpicture} environment. This example also shows that, by
  default, the tableau boxes are square with side lengths of half a
  unit.
  ...
  The \option{label} keys in this example should help in matching the tableaux
  on the left with the corresponding commands on the right.
  ...
  for styling individual boxes in a tableau. Using a key-value
  ...
  colours to the boxes in a tableau or diagram that are placed using a
  residue pattern.
  ...
  As with \TikZ-nodes, use \option{box text} to set the default text
  ...
  colours to the boxes in a tableau. The colours are specified as
  ...
  By default, the colours are assigned cyclically so that the colour
  index:
  \begin{itemize}[nosep]
    \item increases as the column index increases
    \item decreases as the column index decreases.
  \end{itemize}
  ...
  \Diagram[colours={black,white},scale=0.5]{8^8}    \qquad
  \end{example}
  ...
  \textit{before} setting the \option{colours}~(or set the
  \emph{quantum characteristic} \option{e} manually).
  \index{quantum characteristic}
  \index{colours!quantum characteristic}
  ...
  \option{ribbon box style}. The \option{ribbon style} key changes the
  style of the entire ribbon, whereas \option{ribbon box style}
  changes the style of the individual boxes in the ribbon.
  ...
  The difference between this picture and the one given above using
  ...
  \Diagram[entries=contents, snob style={draw=none},
    ...
    snobs={16, 26, 36, 61_\vdots, 62_\vdots, 63_\vdots}
  ] {5^5}
  \end{example}
  ...
  \begin{warning}
    If you use an \aTableau command inside a \textit{scaled} \keyword{tikzpicture}
  ...
  as subscripts and superscripts, in which case they are automatically rescaled
  to slightly smaller sizes. The \option{script} and \option{scriptscript}
  keys give finer control over the sizes of subscripts and subsubscripts,
  respectively, which can be used in conjunction with the \option{scale},
  \option{xscale} and \option{yscale} keys.
  ...
  Set \option{shifted} to \textsf{true} for a shifted tableau or shifted
  diagram. Shifted tableaux, and shifted diagrams, are discussed in more
  detail in \autoref{S:Shifted}, which describes the \keyword{\ShiftedDiagram}
  and \keyword{\ShiftedTableau} commands, which produce pictures like the
  following:
  ...
  The \option{styles} key is a shorthand for defining \TikZ-styles
  ...
  you can save a little bit of typing using the \option{styles} key:
  ...
  to define styles that can be used from this point in the document
  onwards. (Notice the use of \keyword{\aTabset} here. Using \keyword{\tikzset{styles={...}}}
  ...
  \Tableau[russian, tikzpicture={remember picture}]
  ...
  \tikz[remember picture, overlay]{
    \draw[very thick, ->, red](0,0) node[right,align=left]
  ...
  \Tableau[
    ...
    tikz after={
      \draw[ultra thick, red](A-1-3.north east)--(A-1-3.south east)
               --(A-2-1.north east)--(A-3-1.south east)
               --(A-3-1.south west);
   }]{134,567,89}
  \end{example}

  Both before and after hooks are provided because later \TikZ commands are
  drawn over the top of earlier ones, so the order matters. Note that the
  \emph{before} code cannot use the named coordinates \textsf{A-r-c}
  because this code is executed before these nodes are constructed,
  which happens when the tableau is drawn. For more information about
  the drawing order used for tableaux, see \autoref{SS:order}.
  ...
  decreasing sequence. In fact, the \aTableau commands also
  accept diagrams and tableaux of \emph{composition} shape, where the lengths of the
  rows are not necessarily in decreasing order. Diagrams and tableaux of
  composition shape are drawn in exactly the same way as those of
  partition shape.

  \begin{Example}{tableauCompositions}
  \Tableau{45,2751,31 }  \qquad \SkewTableau{1,2}{45,27,31}   \qquad
  \Diagram{1,2,4,1}      \qquad \SkewDiagram{1,2,2,1}{1,2^2,1}
  \end{Example}

  Even though it is possible to draw compositions and tableaux of composition
  shape, compositions are not fully supported in the sense that many of the
  options/keys assume that the diagrams are of partition shape. In addition,
  some options assume that tableaux and diagrams do not contain empty rows
  and that the diagrams are \textit{connected}, in the sense that their
  boundary can be drawn without lifting the pen off the page.  (For
  disconnected tableaux and diagrams, see the \keyword{\Multidiagram}
  and \keyword{\Multitableau} commands in \autoref{S:Multitableau}.)

  % --------------------------------------------------------------
  \subsubsection{Tableaux drawing order}
  \label{SS:order}
  ...
  almost exactly the same syntax as the \keyword{\Tableau} command:
  ...
  environment (and arbitrary $\TikZ$-coordinates can be used).  The
  \keyword{\Diagram} command allows the \emph{partition} to be specified
  as either a comma-separated list of \emph{weakly decreasing positive
  integers}, or using \emph{exponential notation}:
  \index{exponential notation}
  \index{partition!exponential notation}
  ...
  four different conventions for diagrams, with the \option{english}
  convention being the default:
  ...
  \index{residues}
  \index{first tableau}
  \index{tableau!columns}
  \index{tableau!contents}
  \index{tableau!first}
  \index{tableau!hooks}
  \index{tableau!last}
  \index{tableau!ladders}
  \index{tableau!rows}
  ...
  \option[]{entries=value}[columns, contents, first, hooks, last, ladders, residues, rows]
  \option*[]{charge=offsets}[integer offset]

  In each case, use \option{charge} to offset the generated entries. The
  possible choices for \option{entries} are:

  \begin{itemize}

  \item\option{entries=columns}: Prints the diagram where each box is
    labelled by its column index.

  \begin{example}{diagramEntriesColumns}
  \Diagram[entries=columns]{3,2,2} \qquad
  \Diagram[french, entries=columns]{2^2,1} \qquad
  \Diagram[entries=columns,charge=3]{2,1^2}
  \end{example}
  \index{tableau!columns}
  ...
  \Diagram[entries=contents,charge=-3]{2,1^2}
  \end{example}
  \AddKeyLink{contents}
  \AddKeyLink{residues}
  \AddKeyLink{charge}
  ...
  In mathematics-mode, \LaTeX{} typesets minus signs as operators, which
  makes the minus sign for negative contents too wide when they appear inside a
  tableau. To correct for this, \aTableau uses the
  \keyword{\shortminus} command for negative content
  values.
  More explicitly, when using \option{entries=contents}, a content of
  minus one inside a tableau is typeset using \keyword{\shortminus1},
  which looks like~$\shortminus1$ instead of~$-1$.
  ...
  \Diagram[russian, entries=first,charge=2]{3^2,2}
  \end{example}
  \index{first tableau}
  \index{tableau!first}
  \index{entries!first}
  ...
  \item\option{entries=ladders}: Prints the \emph{ladder numbers} of the
  boxes in the diagram. The ladder number of the box in row~$r$ and
  column~$c$ is $c-r+e(r-1)$, so the boxes on the same ladder on lines
  of slope $e-1$ in the diagram (when using the English convention). You
  also need to set the quantum characteristic \option{e}.

  \begin{example}{diagramLadders}
  \Diagram[entries=ladders, e=3]{4,3^3} \qquad
  \Diagram[entries=ladders, e=4]{4,3^3}
  \end{example}
  \index{ladder tableau}
  \index{tableau!ladder}
  \index{entries!ladder}

  Use \option{conjugate} for the ``conjugate'' ladders of slope $1/(e-1)$.

  \begin{example}{diagramConjugateLadders}
  \Diagram[entries=ladders,e=3,conjugate]{4^3,1}\qquad
  \Diagram[entries=ladders,e=4,conjugate]{4^3,1}
  \end{example}

  Ladders are only defined for \option{cartan=A}.
  ...
  labelled by its \emph{$e$-residue}, which is the row index minus the
  ...
  \Diagram[entries=residues, charge=1,
      colours={white,cyan!20,teal!50}]{3,2,1}
  \end{example}

  \item\option{entries=rows}: Prints the diagram where each box is
    labelled by its row index.

  \begin{example}{diagramEntriesRows}
  \Diagram[australian, entries=rows]{2^2,1} \qquad
  \Diagram[entries=rows,charge=2]{3,2,2}
  \end{example}
  \index{tableau!rows}
  ...
  environment and arbitrary \TikZ-coordinates can be used.  The
  partitions for the inner and outer shapes can either be given as a
  comma-separated list of positive integers, or using exponential
  notation for repeated parts.

  Skew tableaux and skew diagrams should always be connected. Use
  ...
  \SkewTableau[skew boxes] {2,1^2}{12,345,67} \qquad
...
\SkewDiagram[entries=columns]{2,1}{2^2,1}\qquad\SkewDiagram[entries=contents]{2,1}{2^2,1}\quad
\SkewDiagram[entries=first]{2,1}{2,2,1}\quad\SkewDiagram[entries=last]{2,1}{2^2,1}\quad
\SkewDiagram[entries=residues,e=3]{2,1}{2^2,1}\quad\SkewDiagram[entries=rows]{2,1}{2,2,1}
\end{Example}
  ...
  \begin{example}{skewEntriesBoxes}
  ...
  \Tableau[cover={3^3}, french, cover border]
    {12[R]3,45,67}
  \end{example}
  ...
  commands are used inside a \keyword{tikzpicture} environment, and
  arbitrary \TikZ-coordinates can be used. The partition in
  a~\keyword{\ShiftedDiagram} can be given using exponential notation
  and the tableau specification for a \keyword{\ShiftedTableau} can
  include the usual optional style prefixes.
  ...
  \begin{example}{tabloidSkew} \Tabloid[skew={2,1}]{19{11},29{10},6}
    \qquad \Tabloid[conjugate, shifted]{19{11}7,29{10},6} \end{example}

  Alternatively, you can draw tableaux like these using the
  \keyword{\SkewTableau} and \keyword{\ShiftedTableau} commands together
  with the \option{tabloid} key. (Under the hood, this results in exactly
  the same pictures being drawn.)

  \begin{example}{skewTabloid}
  \SkewTableau[tabloid]{2,1}{19{11},29{10},6} \qquad
  \ShiftedTableau[tabloid,conjugate]{19{11}7,29{10},6}
  \end{example}
  ...
  \keyword{tikzpicture} environment, and arbitrary \TikZ-coordinates can
  be used. All of the keys for the \keyword{\Tableau} command can be
  used with \keyword{\RibbonTableau}, so we refer to \autoref{S:Tableau}
  ...
  the individual boxes in the ribbon. The (optional) style for an
  entire ribbon is given inside parentheses $(...)$ at the
  \textit{beginning} of the ribbon specification. The ribbons in the
  last example are much easier to see if we add some \keyword{(...)}-styling:
  ...
  the example illustrate, the style of an individual box in the ribbon
  takes precedence over the \keyword{(...)}-style of the entire ribbon.
  ...
  Topologically, the boundary of a generalised ribbon is a disjoint union of (oriented) circles.
  ...
  \begin{example}[righthand width=0.65\textwidth]{ribbonTableauCute}
  ...
  separated list, or using exponential notation.

  \begin{example}{RibbonShape}
  \RibbonTableau[shape={8^2,6^2}]{26crcc}
  \end{example}

  This picture could also be drawn using \keyword{\Diagram[ribbons={26crcc}]{8^2,6^2}}.
  ...
  notation for multipartitions. Styling can be added to the entries in
  a \keyword{\Multitableau} in exactly the same way that style
  ...
  used.  In many cases, the values of multitableaux and
  multidiagrams keys can be separated by the pipe symbol \keyword{|} to
  give the corresponding key values for the component tableaux and
  diagrams.

  \option[-]{english}<multitableau>
  \option*[]{french}<multitableau>
  \option*[]{russian}<multitableau>
  \option*[]{australian}<multitableau>
  ...
  \option[]{dotted cols=|-separated column indices}[$|$ and comma-separated list of columns]<multitableau>
  \option*[]{dotted rows=|-separated row indices}[$|$ and comma-separated list of rows]<multitableau>

  Dotted rows and columns for multidiagrams (and multitableaux) can be
  added using $|$-separated lists of row and column indices in each
  component. This makes it possible to draw multidiagrams,
  where the number of rows and columns are not specified completely.

  \begin{example}{multiDottedRows}
  \Multidiagram[scale=0.9,
    dotted rows=2||2, dotted cols=||2
  ]{ 3,2^2 | | 4,3^2 }
  \end{example}

  The \option{dotted rows} and \option{dotted cols} keys for
  multidiagrams work in exactly the same way as they do for diagrams.
  ...
  \option{text boxes} is true). To add text for empty components, use
  \option{empty=\textbackslash text\{...\}}, or equivalent if you are
  not using \ctan{amsmath}.

  \option[]{entries=value}[columns, contents, first, hooks, last, residues, rows]<multitableau>
  ...
  values for \option{entries}).

  \nopagebreak[0]
  The possible choices are:
  \begin{itemize}

    \item\option{entries=columns}: Prints the tableau that has each box
      labelled by its column index in each component.

  \begin{example}{multiEntriesColumns}
  \Multidiagram[entries=columns]{3,1^2|2^2,1}
  \end{example}

  Use \option{charge}, with a $|$-separated list of entries, to
  offset the column indices in each component.

  \begin{example}{multiEntriesChargedColumns}
  \Multidiagram[entries=columns, charge=0|3]{3,1^2|2^2,1}
  \end{example}

  \item\option{entries=contents}: Prints the tableau where each box is
  labelled by its \emph{content}, which is the row index minus the
  column index.
  ...
  \item\option{entries=ladders}: Prints the ladder numbers in the
  diagram, which are lines of slope $e=1$. Set the quantum
  characteristic with~\option{e}.

  \begin{example}{multiLadders}
  \Multidiagram[entries=ladders, e=2, charge=2|1]{3,2,2|3^3}
  \end{example}
  ...
  \end{example}
    ...
    \item\option{entries=rows}: Prints the tableau in which each box is
      replaced by its row index in each component. As with
      \option{entries=columns}, use \option{charge} with a $|$-separated
      list entries to offset the row indices in each component.

  \begin{example}{multiEntriesRows}
  \Multidiagram[entries=rows]{3,1^2|2^2,1}
  \end{example}
  ...
  component diagrams in multitableaux and multipartitions. By default,
  ...
  \begin{example}{multiSkewCover}
  \Multitableau[cover={3^3|4,3,2}]{123,45|67,89,{10}} \par
  \Multidiagram[skew={|0,1}, cover={|4,3}]{ 3,2 | 3,1 }
  \end{example}

  As this example shows, a covering partition specified by
  \option{cover} is not changed if \option{shifted} or \option{skew}
  are also used.
  ...
  \keyword{tikzpicture} environment, and arbitrary \TikZ-coordinates can
  be used.  The number of runners is a
  ...
  partition, which can be given as a list of positive integers
  ...
  \href{https://tex.stackexchange.com/a/85750/234252}{stack exchange post},
  ...
  (unlike tableaux, both the row and column indices start from~$0$).
  Arbitrary \TikZ-coordinates can be used. The following example
  demonstrates how abacuses are placed using $(x,y)$-coordinates inside
  a \keyword{tikzpicture} environment. The example also shows that, by
  default, the beads and runners in an abacus are half a unit apart.
  ...
  or keys, for tableaux and diagrams can be applied in exactly the same
  way to abacuses, so we do not discuss all of these options here. For
  example, the \keyword{\Abacus} command accepts the following keys:
  ...
  Just like tableaux, abacuses have four different conventions.  By
...
\Abacus[north,entries=beads]{2}{4^2,2,1}\qquad\Abacus[west,entries=beads]{2}{4^2,2,1}
  ...
  bottom of the abacus, using the following of these six symbols:

  \begin{center}
    \rowcolors{2}{}{LightSkyBlue!10}
    \begin{tabular}{Oll} \toprule
      \multicolumn1l{\textsf{Character}} & \textsf{Symbol} & \textsf{Meaning} \\ \midrule
      $-$    & dash         & Draw a perpendicular line that extends beyond the first and last runners\\
      \_\!\_ & underscore   & Draw a perpendicular line between the first and last runners\\
      .      & dot          & Draw ellipses at the end of the runners\\
      *      & asterisk     & Draw ellipses and arrows at the end of the runners\\
      $|$    & pipe         & No decoration\\
      $>$    & greater than & Draw arrows at the end of the runners\\
      \bottomrule
    \end{tabular}
  \end{center}
  ...
  all possibilities, but here are some examples.
 ...
 \begin{example}{abacusEndsEast}
  ...
  the style of the abacus ends, but \option{abacus ends style} overrules
  the \option{runner style}.
  ...
  and~\_\!\_ settings for \option{abacus ends}. There is also a
  \option{framed} key, which works better with the
  \option{traditional} style, that is discussed below.
  ...
  at the ``top'' of the abacus.\footnote{Throughout this section, the
  ...
  The \option{bead} key sets  the bead colour. That is, \option{bead} is a descriptive short-hand for
  the \TikZ-key \keyword{ball color}.

  \begin{example}{abacusBead}
  \Abacus{2}{2^3,1} \qquad
  \Abacus[bead=black]{3}{4^3,[bead=blue]2,1} \qquad
  \Abacus[bead=red]{4}{4^3,[ball color=blue]2,1}
  \end{example}

  The \option{bead} key changes the colour of every bead on the
  abacus. As shown, the colour of individual beads can also be changed by
  ...
  on the right are flat. The \option{traditional} key provides another way
  ...
  When \option{beta numbers} is used, the numbers in the bead
  ...
  The \option{runner labels} key adds labels to each runner. An
  error is given if the number of labels does not match the number of
  ...
  As the first example illustrates, if you use the \option{unshaded} key then
  ...
  key is used, the \keyword{\Abacus} command draws a more traditional
  abacus, where the runners look like rods, and the beads look like
  discs. This takes up more space, but the result is more visually
  appealing, except possibly for the choice of colours!
  ...
  Please
  \href{https://\aTableauMeta{repository}/issues}{report any problems}.
  ...
  argument, the elements of the pictures can be styled in the usual
  ways, and they accept \ctan{beamer} overlay specifications.
...
\end{Example}
  ...
  \uncover<4,5>{\Abacus{3,2}}            \invisible<2-4>{\Tableau<2-4>{12345,789}}
  \end{Example}
  ...
  must be valid colours, as recognised by the \ctan{xcolor} package.
  Setting \option{colour theme} to anything other than one of the names of the three
  colour schemes, or to a list of six colours, gives an error.
  ...
  options apply to tableaux (and diagrams), or to abacuses~--- or both!
  Options starting with \keyword{(no)} are pairs of boolean options and
  their inverses. If these options are not given a true or false value, then
  they are implicitly set to true. Although not explicitly listed, spelling
  ...
  \begin{xltabular}{0.9\textwidth}{l>{\raggedright}Xcc}\toprule
    ...
    \option{east}              & Make abacus grow towards the east                   & \No  & \Yes\\
    ...
    \option{row label style}   & Set the style of the abacus labels                  & \No  & \Yes \\
    ...
    \option{runner label style}& Set the style of the abacus labels                  & \No  & \Yes \\
  ...
  \begin{center}
    ...
    \begin{xltabular}{0.9\textwidth}{ll}\toprule
       ...
       aTableau/abacusEnds      & Tops and bottoms of abacus runners\\
       aTableau/abacusLabel     & Base abacus label style\\
       ...
       aTableau/delimiter       & Base delimiter style in multishapes\\
       ...
       aTableau/leftDelimiter   & Left delimiter in a multishapes\\
       ...
       aTableau/rightDelimiter  & Right delimiter in a multishapes\\
       aTableau/rowLabel        & Style of abacus row labels\\
       aTableau/runnerLabel     & Style of abacus runner labels\\
       aTableau/separatorLine   & Separator lines in multishapes\\
       aTableau/separatorSymbol & Separators in multishapes\\
       ...
       aTableau/tableauLabel    & Tableau labels \\
  ...
  key-value interface, which is the recommended approach. Changes made to
  these styles are not guaranteed to be compatible with future releases of
  the \aTableau package.

  % --------------------------------------------------------------
  \subsection{\aTableau and the \arXiv}

  The \arXiv now plans to update its \TeXLive distribution annually.  As
  of writing, it is using \TeXLive 2025 by default, so papers using
  \aTableau should compile on the \arXiv. The \arXiv will not always
  have the latest version of \aTableau installed.  If your document does
  not compile on the \arXiv due to \aTableau, check to see if it is
  using the latest version of \aTableau. If only an older version of the
  package is available, then try uploading the latest release of the
  \aTableau style file,
  ...
  package to construct some non-trivial real-world examples. In some cases
  we have taken the liberty of making small modifications where we felt
  these improved on the original.
  ...
  \Diagram[french, no boxes, scale=0.7,
    ribbons={*46_\alpha}]{10,7,6^2,3,2}
  \end{example}
  ...
  The next example, from \cite{Muniz}, is notable mainly for the
  ...
  tableaux boxes; see \autoref{E:NodeNames}. As a result, explicit
  \TikZ diagram coordinates are rarely needed in these pictures.

  The first of these examples, which is also from
  \cite{BowmanDeVisscherHaziStroppel:Arc}, is a little more involved.
  The ``hard work'' in drawing this picture diagram is done using custom
  \TikZ-styles, call \textit{pics}, that are a good way of adding repeating
  features to a \keyword{tikzpicture}. The wavy up and down strings in this
  picture are down using pics with the same name, which then use the
  \textit{bendy string} pic to draw the actual strings. These pics take
  as input the string colour, the starting box, the list of the boxes
  that the string passes through, and the final box in the string.
...
\tikzset{ % reusable keys for drawing the strings in the diagram using named coords
   ...
   pics/up string/.style n args = {4}{  % {colour}{first}{middle}{last}
     bendy string={#1}{#2}{#3}{#4}{T}{north}{Cup}{Cap}
   },
   pics/down string/.style n args = {4}{% {colour}{first}{middle}{last}
     bendy string={#1}{#2}{#3}{#4}{O}{south}{Cap}{Cup}
   },
   pics/bendy string/.style n args = {8}{%
    % {colour}{first coord}{middle coords}{last coord}{O/T}{N/S}{Cup/Cap}{Cap/Cup}
    code = {
      \def\last{#2}                                    % should not be necessary
      \draw[thick,#1](#5-|#2.#6 west)node[below=-3.5]{$\wedge$}--(#2.#6 west)
          foreach [remember=\pt as \last (initially #2)] \pt in {#3}
            {to[#7](\last.#6 east) to[#8] (\pt.#6 west)}
             to[#7] (#4.#6 east) -- (#4.#6 east|-#5)node[above=-3.5]{$\vee$};
      }
   },
}
\begin{tikzpicture}[red ribbon/.style={fill=red!40,opacity=0.4}]
 ...
 \pic at (T) {up string={gray}{A-1-5}{}{A-1-5}};           % up strings
 \pic at (T) {up string={gray}{A-4-4}{}{A-4-4}};
 \pic at (T) {up string={gray}{A-5-2}{A-4-3,A-3-4}{A-3-4}};
 \pic at (T) {up string={gray}{A-5-3}{}{A-5-3}};
 \pic at (T) {up string={red}{A-5-1}{A-4-2,A-3-3,A-2-4}{A-2-4}};
 \pic at (O) {down string={gray}{A-1-1}{}{A-1-1}};         % down strings
 \pic at (O) {down string={gray}{A-2-1}{A-1-2}{A-1-2}};
 \pic at (O) {down string={gray}{A-3-1}{A-2-2,A-1-3}{A-1-3}};
 \pic at (O) {down string={gray}{A-4-1}{A-3-2,A-2-3,A-1-4}{A-1-4}};
 \pic at (O) {down string={gray}{A-5-1}{A-4-2,A-3-3,A-2-4,A-1-5}{A-1-5}};
\end{tikzpicture}
\end{Example}

The next example demonstrates how to draw a multipartition and its associated
...
\begin{tikzpicture}[red string/.style={red, ultra thick},
    solid/.style={blue, thick}, ghost/.style={gray} ]
  ...
  { % add solid and ghost strings from the tableau to the KLRW idempotent
...
are significantly different from the examples we have already considered.
One exception is a paper of Fayers and Putignano, on centraliser subalgebras
of the symmetric groups, that uses abacuses like the
following~\cite[page~305]{FayersPutignano:CentraliserAlgebras}:

\begin{Example}{FayersPutignano}
% \usetikzlibrary{arrows.meta}
\aTabset{colour theme=classic,
  tikzpicture={>={Stealth}}, tick length=0.1,
  styles={
    semi={shape=semicircle,inner sep=0pt,minimum height=0.7mm},
    R={semi,bead=red,shape border rotate=270,anchor=west},
    B={semi,shape border rotate=90,anchor=east},
    E={text=blue,unshaded,shift={(0.25,-0.1)}}, Cap={out=30,in=150},
    rarr={thick,red, <-}, barr={thick,blue,->}, % red and blue arrows
  }
}
...
tikz after={ % add the red and blue arrows
  \draw[barr](A-4-1) to[Cap](A-4-5.base);
  \draw[barr](A-4-12) to[Cap](A-4-16.base);
  \draw[barr](A-6-5)node[below left]{$c_{s-1}$}to[out=30,in=180]+(2.2,0.5);
  \draw[barr](A-6-16)to[out=30,in=180]+(2.2,0.5);
  \draw[barr,<-](A-7-1.base)to[out=150,in=0]+(-1,0.4);
  \draw[barr,<-](A-5-12.base)to[out=150,in=0]+(-1,0.4);
...
}]{19}{|[E]4_{d_1},4_b,4,4_{c_s}||2,[R]2,[B]1||5,[E]4_{d_s}||4,[B]1,[R]1|||||
         [E]4_{d_3},4_{c_2}||2,[R]0,[B]0||5_{c_1},[E]4_{d_2}||4,[B]1,[R]0}
\end{Example}

  The most interesting features of this example are the use of styles to change the
  shape of some abacus beads into half-spheres (using the \textsf{R}
  and \textsf{B} styles), and to create ``empty'' beads (using the \textsf{E}
  style, which also shifts the bead text to the bottom right of the bead
  position). Named coordinates are used to draw the arrows between beads using the
  \option{tikz after} key. Notice also that the abacus is specified as a quotient
  (\autoref{SS:quotients}).
  ...
  page:
  \begin{center}
    \href{https://\aTableauMeta{repository}/issues}{\aTableauMeta{repository}/issues}.
  \end{center}
  Please provide as much detail as possible when submitting requests.

  Bug reports must be accompanied by a \textit{minimal working
  example}, which is the smallest amount of \LaTeX\ code that
  demonstrates the problem and compiles---unless you are reporting a
  problem that gives a compilation error, in which case your example code
  should reproduce the error. This is necessary because it is very difficult
...
\textsc{A.~Mathas and D.~Tubbenhauer},
  \href{http://dx.doi.org/10.1112/jlms.12706}{\textit{Cellularity
